...
function BusinessIntelligence(){
 const[mode,setMode]=useState('hoy');const[date,setDate]=useState(today());const[bi,setBi]=useState({tickets:[],lines:[],sync:null,articles:new Map(),costMap:new Map()});const[clock,setClock]=useState([]);const[realCosts,setRealCosts]=useState({fixed:0,variable:0,laborAccrued:0,payments:0,hours:0});const[loading,setLoading]=useState(false);const r=rangeDates(mode,date);
 ...
 return <div className="businessIntelligencePro"><div className="card hero intelligenceHero"><div><span className="sectionEyebrow">INTELIGENCIA REAL DEL NEGOCIO</span><h2>Ventas, margen y beneficio</h2><p>{r.label}. Todos los rankings y franjas usan este mismo periodo.</p></div><div className="row controls intelligenceRange">{buttons.map(([id,label])=><button key={id} className={mode===id?'active':''} onClick={()=>setMode(id)}>{label}</button>)}<input type="date" value={date} onChange={e=>{setDate(e.target.value);setMode('fecha')}}/><button onClick={load}>{loading?'Cargando...':'Actualizar'}</button></div></div><div className="grid intelligenceKpis"><div className="card kpi"><span>Ventas</span><b>{money(financial.revenue)}</b></div><div className="card kpi"><span>Coste productos</span><b>{money(financial.cost)}</b></div><div className="card kpi"><span>Margen bruto</span><b>{money(financial.profit)}</b><small>{financial.revenue?`${financial.margin.toFixed(1)}%`:'—'}</small></div><div className="card kpi"><span>Personal devengado</span><b>{money(realCosts.laborAccrued)}</b><small>{Number(realCosts.hours||0).toFixed(1)} h reales</small></div><div className="card kpi"><span>Gastos fijos imputados</span><b>{money(realCosts.fixed)}</b></div><div className="card kpi"><span>Gastos variables</span><b>{money(realCosts.variable)}</b></div><div className="card kpi realProfitKpi"><span>Beneficio real estimado</span><b className={financial.realProfit>=0?'ok':'bad'}>{money(financial.realProfit)}</b><small>{financial.revenue?`${financial.realMargin.toFixed(1)}% sobre ventas`:'—'}</small></div><div className="card kpi"><span>Tickets / ticket medio</span><b>{daily.tickets}</b><small>{money(daily.ticket_medio)}</small></div></div>{financial.unknownRevenue>0&&<div className="warnBox">Hay {money(financial.unknownRevenue)} en ventas con coste pendiente. Configura esos artículos en Rentabilidad → Márgenes editables para completar el beneficio real.</div>}<div className="grid"><PredictionBox mode={mode} date={date} tickets={bi.tickets}/><div className="card"><h2>📈 Ventas por hora</h2><SalesByHour tickets={bi.tickets}/></div></div><ShiftProfitability tickets={bi.tickets} lines={bi.lines} clockRows={clock} costMap={bi.costMap}/><ProductRanking lines={bi.lines} articles={bi.articles} costMap={bi.costMap} onCostSaved={load}/><DailyReport summary={daily} lines={bi.lines} tickets={bi.tickets} clockRows={clock} period={r} costMap={bi.costMap} articles={bi.articles}/></div>;
}
