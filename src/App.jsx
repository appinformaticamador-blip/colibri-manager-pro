...
function ProductRanking({lines,articles,costMap}){
 const[sort,setSort]=useState('qty');const[selected,setSelected]=useState(null);
 const top=productRank(lines,sort,articles).map(p=>{const info=costMap?.get?.(String(p.code));const cost=info&&!info.excluded&&info.cost!==null?Number(info.cost)*p.qty:null;const profit=cost===null?null:p.total-cost;return {...p,cost,profit,margin:profit===null||!p.total?null:(profit/p.total)*100,unitCost:info?.cost??null}}).slice(0,30);
 return <><div className="card"><div className="row between intelligenceCardTitle"><div><h2>🍽️ Ranking y beneficio por producto</h2><p className="mutedText">Calculado para el rango seleccionado arriba. Pulsa un producto para ver su detalle económico.</p></div><div><button className={sort==='qty'?'active':''} onClick={()=>setSort('qty')}>Unidades</button><button className={sort==='total'?'active':''} onClick={()=>setSort('total')}>Facturación</button></div></div><div className="tableScroll"><table className="clickableTable"><thead><tr><th>#</th><th>Producto</th><th>Unidades</th><th>Ventas</th><th>Coste</th><th>Beneficio</th><th>Margen</th></tr></thead><tbody>{top.map((p,i)=><tr key={p.name} onClick={()=>setSelected(p)}><td>{i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1}</td><td><b>{p.name}</b>{p.code&&<small className="mutedCode"> {p.code}</small>}</td><td>{p.qty.toFixed(2)}</td><td>{money(p.total)}</td><td>{p.cost===null?<span className="warningText">Sin coste</span>:money(p.cost)}</td><td><b className={p.profit===null?'info':'ok'}>{p.profit===null?'Pendiente':money(p.profit)}</b></td><td>{p.margin===null?'—':`${p.margin.toFixed(1)}%`}</td></tr>)}</tbody></table></div>{top.length===0&&<p>No hay líneas de productos para este periodo.</p>}</div>{selected&&<div className="modal" onClick={()=>setSelected(null)}><div className="card intelligenceModal" onClick={e=>e.stopPropagation()}><div className="row between"><div><span className="sectionEyebrow">RENTABILIDAD DEL PERIODO</span><h2>{selected.name}</h2><p>Código {selected.code||'sin código'} · {selected.ticketCount} tickets</p></div><button className="red" onClick={()=>setSelected(null)}>Cerrar</button></div><div className="intelligenceDetailGrid"><p><span>Unidades vendidas</span><b>{selected.qty.toFixed(2)}</b></p><p><span>Facturación</span><b>{money(selected.total)}</b></p><p><span>Coste unitario</span><b>{selected.unitCost===null?'Sin configurar':money(selected.unitCost)}</b></p><p><span>Coste total</span><b>{selected.cost===null?'Sin configurar':money(selected.cost)}</b></p><p className="featured"><span>Beneficio estimado</span><b>{selected.profit===null?'Coste pendiente':money(selected.profit)}</b></p><p><span>Margen</span><b>{selected.margin===null?'—':`${selected.margin.toFixed(1)}%`}</b></p></div>{selected.cost===null&&<p className="warnBox">Configura el coste manual de este artículo en Rentabilidad → Márgenes editables para obtener su beneficio.</p>}</div></div>}</>;
}
function ShiftProfitability({tickets,lines,clockRows,costMap}){
 const[selected,setSelected]=useState(null);const linesByCab=new Map();(lines||[]).forEach(l=>{const k=String(l.cab_id);if(!linesByCab.has(k))linesByCab.set(k,[]);linesByCab.get(k).push(l)});
 ...
 const byEmp={};(clockRows||[]).slice().reverse().forEach(r=>{const k=r.employee_id||r.employee_name;byEmp[k]=byEmp[k]||[];byEmp[k].push(r)});Object.values(byEmp).forEach(list=>{let open=null;list.forEach(r=>{const typ=String(r.type).toLowerCase();if(typ==='entrada')open=r;else if(typ==='salida'&&open){const a=decimalHour(open.created_at),b=decimalHour(r.created_at);if(b>a)shiftStats.forEach(s=>s.staffHours+=overlapHours(a,b,s.start,s.end));open=null}})});
...
function BusinessIntelligence(){
 const[mode,setMode]=useState('hoy');const[date,setDate]=useState(today());const[bi,setBi]=useState({tickets:[],lines:[],sync:null,articles:new Map(),costMap:new Map()});const[clock,setClock]=useState([]);const[loading,setLoading]=useState(false);const r=rangeDates(mode,date);
 useEffect(()=>{load();const t=setInterval(load,60000);return()=>clearInterval(t)},[mode,date]);
 async function load(){if(!supabase)return;setLoading(true);const [{tickets,lines,sync,articles},{data:clockData},costMap]=await Promise.all([loadSalesRange(r.from,r.to),supabase.from('clock_records').select('*').gte('created_at',r.from+'T00:00:00').lt('created_at',r.to+'T00:00:00').order('created_at',{ascending:true}).limit(5000),loadProfitabilityCostMap()]);setBi({tickets,lines,sync,articles,costMap});setClock(clockData||[]);setLoading(false)}
 const daily=summarizeTickets(bi.tickets);const financial=periodFinancials(bi.lines,bi.costMap);
 const buttons=[['hoy','Hoy'],['ayer','Ayer'],['tresdias','3 días'],['semana','7 días'],['treinta','30 días'],['mes','Mes'],['anio','Año']];
 return <div className="businessIntelligencePro"><div className="card hero intelligenceHero"><div><span className="sectionEyebrow">INTELIGENCIA REAL DEL NEGOCIO</span><h2>Ventas, margen y beneficio</h2><p>{r.label}. Todos los rankings y franjas usan este mismo periodo.</p></div><div className="row controls intelligenceRange">{buttons.map(([id,label])=><button key={id} className={mode===id?'active':''} onClick={()=>setMode(id)}>{label}</button>)}<input type="date" value={date} onChange={e=>{setDate(e.target.value);setMode('fecha')}}/><button onClick={load}>{loading?'Cargando...':'Actualizar'}</button></div></div><div className="grid intelligenceKpis"><div className="card kpi"><span>Ventas</span><b>{money(financial.revenue)}</b></div><div className="card kpi"><span>Coste productos</span><b>{money(financial.cost)}</b></div><div className="card kpi"><span>Beneficio bruto</span><b>{money(financial.profit)}</b></div><div className="card kpi"><span>Margen conocido</span><b>{financial.revenue?`${financial.margin.toFixed(1)}%`:'—'}</b></div><div className="card kpi"><span>Tickets</span><b>{daily.tickets}</b></div><div className="card kpi"><span>Ticket medio</span><b>{money(daily.ticket_medio)}</b></div></div>{financial.unknownRevenue>0&&<div className="warnBox">Hay {money(financial.unknownRevenue)} en ventas con coste pendiente. Configura esos artículos en Rentabilidad → Márgenes editables para completar el beneficio real.</div>}<div className="grid"><PredictionBox mode={mode} date={date} tickets={bi.tickets}/><div className="card"><h2>📈 Ventas por hora</h2><SalesByHour tickets={bi.tickets}/></div></div><ShiftProfitability tickets={bi.tickets} lines={bi.lines} clockRows={clock} costMap={bi.costMap}/><ProductRanking lines={bi.lines} articles={bi.articles} costMap={bi.costMap}/><DailyReport summary={daily} lines={bi.lines} tickets={bi.tickets} clockRows={clock} period={r}/></div>;
}
