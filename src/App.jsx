...
function SyncStatusCard(){
 ...
 return <div className={'card syncStatus '+(live?'live':'syncing')}>
  <div className="row between"><div><h2>{live?'ðŸŸ¢ ACTUALIZADO 100%':'ðŸŸ¡ SINCRONIZANDO'}</h2><p>{st?.message||'Esperando datos de ColibrÃ­ Sync'}</p></div><div className="syncPct">{pct.toFixed(1)}%</div></div>
  ...
  <div className="syncGrid">
   ...
   <span>Ãšltimo CAB_ID <b>{Number(st?.last_cab_id||0).toLocaleString('es-ES')}</b></span>
   <span>Ãšltima sync <b>{secondsAgo(st?.updated_at)}</b></span>
...
function SalesCards({daily,sync}){return <div className="grid">
 <div className="card kpi"><span>ðŸ’¶ Ventas</span><b>{money(daily?.total)}</b></div>
 <div className="card kpi"><span>ðŸŽ« Tickets</span><b>{daily?.tickets||0}</b></div>
 <div className="card kpi"><span>ðŸ“Š Ticket medio</span><b>{money(daily?.ticket_medio)}</b></div>
 <div className="card kpi"><span>ðŸ’µ Efectivo</span><b>{money(daily?.efectivo)}</b></div>
 <div className="card kpi"><span>ðŸ’³ Tarjeta</span><b>{money(daily?.tarjeta)}</b></div>
 <div className="card kpi"><span>ðŸ”„ Ãšltima sync</span><b>{sync?.synced_at?new Date(sync.synced_at).toLocaleTimeString('es-ES'):'-'}</b></div>
 </div>}
...
const SHIFT_DEFS=[
 {id:'desayuno',name:'â˜• Desayuno',start:8,end:12},
 {id:'almuerzo',name:'ðŸ½ï¸ Almuerzo',start:12,end:16},
 {id:'tarde',name:'ðŸŒ‡ Tarde',start:16,end:20},
 {id:'cena',name:'ðŸŒ™ Cena',start:20,end:23.5}
];
const EMPLOYEE_HOUR_COST=7;
const SMART_GROWTH_TARGET=0.10; // Objetivo inteligente: +10% sobre histÃ³rico comparable
...
function rangeDates(mode,date){
 ...
 if(mode==='semana') return {from:addDays(today(),-6),to:addDays(today(),1),label:'Ãšltimos 7 dÃ­as'};
...
async function loadSmartGoal(date,currentTotal=0){
 ...
 let base=0,source='';
 if(avgYear>0&&avgSame>0){base=(avgYear*0.6)+(avgSame*0.4);source='aÃ±o anterior + Ãºltimos mismos dÃ­as';}
 else if(avgSame>0){base=avgSame;source='Ãºltimos mismos dÃ­as de la semana';}
...
// GestorÃ­a PRO: lectura sin lÃ­mite de 1000 registros. Supabase devuelve por pÃ¡ginas.
...
async function loadSalesRangeGestoria(from,to,onProgress=null){
 ...
 let lines=[];
 onProgress&&onProgress(`Cargando lÃ­neas de ${cabIds.length.toLocaleString('es-ES')} tickets...`);
 for(let i=0;i<cabIds.length;i+=80){
  const chunk=cabIds.slice(i,i+80);
  const part=await fetchAllPages(()=>supabase.from('numier_ticket_lines').select('*').in('cab_id',chunk).order('cab_id',{ascending:true}),1000,null,'lÃ­neas');
  lines=lines.concat(part);
  onProgress&&onProgress(`Cargando lÃ­neas: ${Math.min(i+80,cabIds.length).toLocaleString('es-ES')} / ${cabIds.length.toLocaleString('es-ES')} tickets`);
 }
 const [{data:syncData},articles]=await Promise.all([
  supabase.from('numier_sync_files').select('*').order('synced_at',{ascending:false}).limit(1),
  loadArticlesMap()
 ]);
 onProgress&&onProgress(`Completado: ${tickets.length.toLocaleString('es-ES')} tickets y ${lines.length.toLocaleString('es-ES')} lÃ­neas.`);
 return {tickets,lines,sync:syncData?.[0]||null,articles};
}
...
function ticketText({ticket,lines,articles,account}){
 const title=account?(account.zona==='barra'?`CUENTA RÃPIDA ${account.mesa}`:`MESA ${account.mesa_numero}`):`TICKET ${ticket?.numdoc||ticket?.cab_id||''}`;
...
function punctualityFor(row,expected){
 if(String(row.type).toLowerCase()==='salida')return {label:'Salida',cls:'exit',icon:'ðŸ”´'};
 const st=expected?.[row.employee_name];if(!st)return {label:'Sin turno',cls:'neutral',icon:'âšª'};
 ...
 if(diff>10)return {label:`+${diff} min`,cls:'late10',icon:'âš ï¸'};
 if(diff>5)return {label:`+${diff} min`,cls:'late5',icon:'ðŸŸ¡'};
 return {label:diff>0?`+${diff} min`:'Puntual',cls:'ok',icon:'ðŸŸ¢'};
}
...
function ProductRanking({lines,articles}){const[sort,setSort]=useState('qty');const top=productRank(lines,sort,articles).slice(0,20);return <div className="card"><div className="row between"><h2>ðŸ½ï¸ Ranking de productos</h2><div><button className={sort==='qty'?'active':''} onClick={()=>setSort('qty')}>Unidades</button><button className={sort==='total'?'active':''} onClick={()=>setSort('total')}>FacturaciÃ³n</button></div></div><table><thead><tr><th>#</th><th>Producto</th><th>Familia</th><th>Unidades</th><th>Total</th><th>â‚¬/ud</th></tr></thead><tbody>{top.map((p,i)=><tr key={p.name}><td>{i===0?'ðŸ¥‡':i===1?'ðŸ¥ˆ':i===2?'ðŸ¥‰':i+1}</td><td><b>{p.name}</b>{p.code&&<small className="mutedCode"> {p.code}</small>}</td><td>{p.family||'-'}</td><td>{p.qty.toFixed(2)}</td><td>{money(p.total)}</td><td>{money(p.avg)}</td></tr>)}</tbody></table>{top.length===0&&<p>No hay lÃ­neas de productos para este periodo.</p>}</div>}
function ShiftProfitability({tickets,clockRows}){
 ...
 return <div className="card"><h2>â° Rentabilidad por turnos</h2><p className="mutedText">Coste empleado configurado: <b>7 â‚¬/h</b></p><table><thead><tr><th>Turno</th><th>Ventas</th><th>Tickets</th><th>Horas personal</th><th>Coste personal</th><th>Ãndice</th></tr></thead><tbody>{shiftStats.map(s=><tr key={s.id}><td><b>{s.name}</b></td><td>{money(s.sales)}</td><td>{s.tickets}</td><td>{s.staffHours.toFixed(1)} h</td><td>{money(s.cost)}</td><td><b className={s.profitIndex>=0?'ok':'bad'}>{money(s.profitIndex)}</b></td></tr>)}</tbody></table></div>
}
function PredictionBox({mode,date,tickets}){const total=tickets.reduce((a,t)=>a+Number(t.total||0),0);const projected=mode==='hoy'? total/Math.max(0.25,(new Date().getHours()+new Date().getMinutes()/60-8)/15.5) : total;return <div className="card"><h2>ðŸ”® PredicciÃ³n rÃ¡pida</h2>{mode==='hoy'?<><p>Ventas actuales: <b>{money(total)}</b></p><p>Si el ritmo se mantiene, cierre estimado: <b>{money(Math.max(total,projected))}</b></p></>:<p>Selecciona <b>Hoy</b> para ver predicciÃ³n de cierre.</p>}<p className="mutedText">La predicciÃ³n mejorarÃ¡ cuando acumulemos mÃ¡s histÃ³rico por dÃ­a de semana.</p></div>}
function BusinessIntelligence(){const[mode,setMode]=useState('hoy');const[date,setDate]=useState(today());const[bi,setBi]=useState({tickets:[],lines:[],sync:null,articles:new Map()});const[clock,setClock]=useState([]);const[loading,setLoading]=useState(false);const r=rangeDates(mode,date);useEffect(()=>{load();const t=setInterval(load,60000);return()=>clearInterval(t)},[mode,date]);async function load(){if(!supabase)return;setLoading(true);const [{tickets,lines,sync,articles},{data:clockData}]=await Promise.all([loadSalesRange(r.from,r.to),supabase.from('clock_records').select('*').gte('created_at',r.from+'T00:00:00').lt('created_at',r.to+'T00:00:00').order('created_at',{ascending:true}).limit(5000)]);setBi({tickets,lines,sync,articles});setClock(clockData||[]);setLoading(false)}const daily=summarizeTickets(bi.tickets);return <div><div className="card hero"><div><h2>ðŸ“Š Inteligencia de Negocio v2.4</h2><p>{r.label}</p></div><div className="row controls"><button className={mode==='hoy'?'active':''} onClick={()=>setMode('hoy')}>Hoy</button><button className={mode==='ayer'?'active':''} onClick={()=>setMode('ayer')}>Ayer</button><button className={mode==='semana'?'active':''} onClick={()=>setMode('semana')}>7 dÃ­as</button><button className={mode==='mes'?'active':''} onClick={()=>setMode('mes')}>Mes</button><input type="date" value={date} onChange={e=>{setDate(e.target.value);setMode('fecha')}}/><button onClick={load}>{loading?'Cargando...':'Actualizar'}</button></div></div><SyncStatusCard/><SalesCards daily={daily} sync={bi.sync}/><div className="grid"><PredictionBox mode={mode} date={date} tickets={bi.tickets}/><div className="card"><h2>ðŸ“ˆ Ventas por hora</h2><SalesByHour tickets={bi.tickets}/></div></div><ShiftProfitability tickets={bi.tickets} clockRows={clock}/><ProductRanking lines={bi.lines} articles={bi.articles}/><DailyReport summary={daily} lines={bi.lines} tickets={bi.tickets} clockRows={clock} period={r}/></div>}
...
function zoneLabel(z){return z==='terraza'?'Terraza':z==='salon'?'SalÃ³n':'Barra'}
...
function serviceForecast({closedToday,totalPending,openCount,avgMin}){
 ...
 let tone='ðŸŸ¢ Servicio tranquilo';
 if(openCount>=12)tone='ðŸŸ¡ Servicio con mucha actividad';
 if(openCount>=18)tone='ðŸ”´ Servicio muy cargado';
 return {potential,uplift,tone,text: pending>0?`Hay ${openCount} cuentas abiertas con ${money(pending)} pendientes. Aplicando una estimaciÃ³n prudente de consumo adicional del 18%, el potencial inmediato del servicio es ${money(potential)}.`:`No hay pendiente de cobro ahora mismo. El servicio estÃ¡ limpio y la previsiÃ³n depende del ritmo de nuevas mesas.`};
}
function ServiceTable({def,account,onOpen}){
 ...
 return <button type="button" className={'serviceTable '+cls} style={{left:def.x+'%',top:def.y+'%'}} title={open?`Mesa ${def.n} Â· ${money(account.total)} Â· ${durationShort(mins)}`:`Mesa ${def.n} libre`} onClick={()=>open&&onOpen(account)}>
...
function TicketModal({cabId,account,onClose}){
 ...
 return <div className="modal" onClick={onClose}><div className="card ticketModal" onClick={e=>e.stopPropagation()}><div className="row between"><h2>{account?(account.zona==='barra'?`Cuenta rÃ¡pida ${account.mesa||account.mesa_numero}`:`Mesa ${account.mesa_numero}`):`Ticket ${detail.ticket?.numdoc||cabId}`}</h2><button className="red" onClick={onClose}>Cerrar</button></div>
  {detail.loading?<p>Cargando ticket...</p>:detail.error?<p className="error">{detail.error}</p>:<>
   ...
   <div className="ticketBox"><h3>BRASERÃA EL COLIBRÃ</h3><p>{account?(account.zona==='barra'?'Cuenta rÃ¡pida':`Mesa ${account.mesa_numero}`):`Ticket ${detail.ticket?.numdoc||cabId}`}</p><table><thead><tr><th>Producto</th><th>Ud</th><th>Total</th></tr></thead><tbody>{detail.lines.map(l=><tr key={l.line_key||l.id}><td>{productNameFromLine(l,detail.articles)}</td><td>{Number(l.cantidad||0)}</td><td>{money(l.importe)}</td></tr>)}</tbody><tfoot><tr><td colSpan="2">TOTAL</td><td>{money(total)}</td></tr></tfoot></table>{detail.lines.length===0&&<p>No hay lÃ­neas cargadas para este ticket.</p>}</div>
...
function EstadoServicio(){
 ...
 return <div className="servicePage"><div className="serviceHeader"><div><span className="pill">ColibrÃ­ ERP PRO 3.1.0 RC2 Â· Beta</span><h1>Estado del Servicio</h1><p>Restaurante en directo Â· mesas abiertas, zonas y pendiente de cobro.</p></div><div className="serviceLive"><b>ðŸŸ¢ EN DIRECTO</b><span>{last?secondsAgo(last):'sin datos'}</span><button onClick={load}>{loading?'Actualizando...':'Actualizar'}</button></div></div>{state.error&&<div className="alertBad">Error: {state.error}</div>}
 <div className="serviceKpis enhanced"><div><span>OcupaciÃ³n total</span><b>{open.filter(o=>o.zona!=='barra').length} / 23</b><em>{occTotal}%</em><OccupancyBar value={occTotal}/></div><div><span>Vendido hoy</span><b>{money(closedToday)}</b><em>facturaciÃ³n cerrada</em></div><div><span>Pendiente de cobro</span><b>{money(totalPending)}</b><em>{open.length} cuentas abiertas</em></div><div><span>Potencial inmediato</span><b>{money(potential)}</b><em>vendido + pendiente</em></div><div className={timeColor}><span>Tiempo medio</span><b>{durationShort(avgMin)}</b><em>cuentas abiertas</em></div></div>
 <div className="card serviceAi"><h2>ðŸ¤– Lectura IA del servicio</h2><p><b>{forecast.tone}</b>. {forecast.text}</p><div className="serviceFormula"><span>Vendido: <b>{money(closedToday)}</b></span><span>+</span><span>Pendiente: <b>{money(totalPending)}</b></span><span>=</span><span>Potencial: <b>{money(potential)}</b></span></div></div>
 <div className="serviceLayout"><main><ServiceZoneMap title="TERRAZA (00-19)" tables={TERRACE_TABLES} openByMesa={openByMesa} onOpen={setSelected}/><ServiceZoneMap title="SALÃ“N / BARRA (20-30)" tables={SALON_TABLES} openByMesa={openByMesa} onOpen={setSelected}/></main><aside className="servicePanel"><div className="sideCard"><h3>OcupaciÃ³n por zona</h3><p><span>Terraza</span><b>{terrace.length}/15</b><em>{occTerrace}%</em></p><OccupancyBar value={occTerrace}/><p><span>SalÃ³n</span><b>{salon.length}/8</b><em>{occSalon}%</em></p><OccupancyBar value={occSalon}/><p><span>Barra</span><b>{barra.length}</b><em>cuentas</em></p></div><div className="sideCard"><h3>Cuentas abiertas</h3><p><span>Terraza</span><b>{terrace.length}</b><em>{money(terrace.reduce((a,o)=>a+Number(o.total||0),0))}</em></p><p><span>SalÃ³n</span><b>{salon.length}</b><em>{money(salon.reduce((a,o)=>a+Number(o.total||0),0))}</em></p><p><span>Barra</span><b>{barra.length}</b><em>{money(barra.reduce((a,o)=>a+Number(o.total||0),0))}</em></p><p className="total"><span>Total pendiente</span><b></b><em>{money(totalPending)}</em></p></div><div className="sideCard"><h3>Ãšltimas aperturas</h3>{open.slice().sort((a,b)=>new Date(b.opened_at)-new Date(a.opened_at)).slice(0,5).map(o=><p key={o.cab_id}><span>{o.zona==='barra'?'Barra':`Mesa ${o.mesa_numero}`}</span><b>{money(o.total)}</b><em>{durationShort(minutesOpen(o.opened_at))}</em></p>)}{open.length===0&&<p><span>No hay cuentas abiertas</span></p>}</div><div className="sideCard"><h3>AuditorÃ­a hoy</h3>{['N','X','G'].map(st=>{const rows=(state.audit||[]).filter(a=>a.estado===st);const sum=rows.reduce((a,r)=>a+Number(r.total||0),0);return <p key={st}><span>{st==='N'?'Borradas manualmente':st==='X'?'Anuladas':'Gastos'}</span><b>{rows.length}</b><em>{money(sum)}</em></p>})}</div><div className="sideCard legend"><h3>Leyenda</h3><p><i className="dot free"></i> Libre</p><p><i className="dot open"></i> Abierta &lt;30 min</p><p><i className="dot notice"></i> 30-60 min</p><p><i className="dot warn"></i> 60-90 min</p><p><i className="dot danger"></i> +90 min</p></div><div className="sideCard about"><h3>Software</h3><p><b>ColibrÃ­ ERP PRO</b><br/>VersiÃ³n 3.1.0 RC2 Â· Beta</p><p>Desarrollado por<br/><b>C.G. 21 S.L.</b><br/>C/ Amador de los RÃ­os, 16<br/>41003 Sevilla<br/>â˜Ž 954 533 502</p></div></aside></div><TicketModal cabId={selected?.cab_id} account={selected} onClose={()=>setSelected(null)}/></div>
}

function DailyReport({summary,lines,tickets,clockRows,period}){const topQty=productRank(lines,'qty')[0];const topMoney=productRank(lines,'total')[0];const totalStaffHours=(clockRows||[]).filter(r=>String(r.type).toLowerCase()==='entrada').length;return <div className="card report"><h2>ðŸ§¾ Informe diario / periodo</h2><div className="reportGrid"><p><span>Ventas</span><b>{money(summary.total)}</b></p><p><span>Tickets</span><b>{summary.tickets}</b></p><p><span>Ticket medio</span><b>{money(summary.ticket_medio)}</b></p><p><span>Producto mÃ¡s vendido</span><b>{topQty?topQty.name:'-'}</b></p><p><span>Mayor facturaciÃ³n</span><b>{topMoney?topMoney.name:'-'}</b></p><p><span>Coste hora</span><b>7 â‚¬/h</b></p></div><button onClick={()=>navigator.clipboard.writeText(`INFORME COLIBRÃ ERP\n${period.label}\nVentas: ${money(summary.total)}\nTickets: ${summary.tickets}\nTicket medio: ${money(summary.ticket_medio)}\nEfectivo: ${money(summary.efectivo)}\nTarjeta: ${money(summary.tarjeta)}\nProducto mÃ¡s vendido: ${topQty?topQty.name:'-'}\nMayor facturaciÃ³n: ${topMoney?topMoney.name:'-'}`)}>Copiar informe</button><p className="mutedText">El informe automÃ¡tico de las 00:00 queda preparado a nivel de datos; de momento tambiÃ©n puedes generarlo y copiarlo desde aquÃ­.</p></div>}


/* COLIBRI_HOTFIX_3_1_X_01
   Sprint: Hotfix 3.1.x-01
   Motivo: Los tabs Cuadrantes, Comparador y Configuracion llamaban a componentes no definidos
...
function Schedule(){
 let data={};
 try{data=JSON.parse(localStorage.colibriSchedule||'{}')}catch{data={}}
 const totalBlocks=Object.values(data||{}).reduce((a,v)=>a+(Array.isArray(v)?v.length:0),0);
 return <div>
  <SafeHotfixCard icon="ðŸ“…" title="Cuadrantes" subtitle="Modulo recuperado por hotfix estable. No rompe el ERP si no hay datos guardados.">
   <div className="grid">
    <div className="card kpi"><span>Bloques guardados</span><b>{totalBlocks}</b></div>
    <div className="card kpi"><span>Estado</span><b>Operativo</b></div>
   </div>
   <p className="mutedText">Hotfix temporal: se restaura la carga del modulo para evitar pantalla blanca. Sprint siguiente: recuperar editor completo de cuadrantes.</p>
  </SafeHotfixCard>
 </div>
}
...
function Compare(){
 return <div>
  <SafeHotfixCard icon="ðŸ“Š" title="Comparador" subtitle="Modulo recuperado por hotfix estable.">
   <div className="grid">
    <div className="card kpi"><span>Estado</span><b>Operativo</b></div>
    <div className="card kpi"><span>Riesgo</span><b>Bajo</b></div>
   </div>
   <p className="mutedText">Hotfix temporal: evita que el ERP se quede en blanco. Sprint siguiente: comparativa real por fechas, tickets, formas de pago y ticket medio.</p>
  </SafeHotfixCard>
 </div>
}
...
function Settings(){
 ...
 return <div>
  <SafeHotfixCard icon="âš™ï¸" title="Configuracion" subtitle="Modulo recuperado por hotfix estable.">
   <div className="grid">
    <div className="card kpi"><span>Supabase</span><b>{hasSupabase?'OK':'Falta ENV'}</b></div>
    <div className="card kpi"><span>Version</span><b>3.1.x</b></div>
    <div className="card kpi"><span>Beta</span><b>Activa</b></div>
   </div>
   <p><b>COLIBRI ERP PRO</b></p>
   <p>C.G. 21 S.L. Â· C/ Amador de los Rios 16 Â· 41003 Sevilla Â· 954533502</p>
   <p className="mutedText">Hotfix temporal: configuracion basica visible y sin riesgo. Sprint siguiente: panel completo de parametros.</p>
  </SafeHotfixCard>
 </div>
}
...
function Brand(){return <div className="brand"><img src="/logo_colibri.png" onError={e=>e.currentTarget.style.display='none'}/><div><h1>ColibrÃ­ ERP</h1><p>BraserÃ­a El ColibrÃ­</p></div></div>}
function Manager(){const[tab,setTab]=useState('dashboard');const tabs=['dashboard','servicio','inteligencia','tpv','gestoria','empleados','fichajes','cuadrantes','comparador','config'];return <div><header><Brand/><nav>{tabs.map(t=><button className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{t}</button>)}</nav></header><section className="page">{tab==='dashboard'&&<Dashboard/>}{tab==='servicio'&&<EstadoServicio/>}{tab==='inteligencia'&&<BusinessIntelligence/>}{tab==='empleados'&&<Employees/>}{tab==='fichajes'&&<ClockPanel/>}{tab==='cuadrantes'&&<Schedule/>}{tab==='comparador'&&<Compare/>}{tab==='tpv'&&<TPV/>}{tab==='gestoria'&&<Gestoria/>}{tab==='config'&&<Settings/>}</section></div>}

function getGreeting(){const h=new Date().getHours();if(h<12)return 'Buenos dÃ­as';if(h<20)return 'Buenas tardes';return 'Buenas noches'}
...
function buildDashboardTimeline(tickets,clockRows,daily,objective){
 ...
 if(firstTicket)events.push({time:firstTicket.hora||firstTicket.created_at,icon:'ðŸ§¾',text:`Primer ticket Â· ${money(firstTicket.total)}`});
 if(lastTicket&&lastTicket!==firstTicket)events.push({time:lastTicket.hora||lastTicket.created_at,icon:'ðŸ§¾',text:`Ãšltimo ticket Â· ${money(lastTicket.total)}`});
 const maxTicket=sortedTickets.reduce((m,t)=>Number(t.total||0)>Number(m?.total||0)?t:m,null);
 if(maxTicket)events.push({time:maxTicket.hora||maxTicket.created_at,icon:'ðŸ’Ž',text:`Ticket mÃ¡s alto Â· ${money(maxTicket.total)}`});
 let acc=0, passed25=false, passed50=false, passed75=false;
 sortedTickets.forEach(t=>{acc+=Number(t.total||0); if(objective>0&&!passed25&&acc>=objective*.25){passed25=true;events.push({time:t.hora||t.created_at,icon:'ðŸŽ¯',text:'25% del objetivo alcanzado'})} if(objective>0&&!passed50&&acc>=objective*.5){passed50=true;events.push({time:t.hora||t.created_at,icon:'ðŸŽ¯',text:'50% del objetivo alcanzado'})} if(objective>0&&!passed75&&acc>=objective*.75){passed75=true;events.push({time:t.hora||t.created_at,icon:'ðŸŽ¯',text:'75% del objetivo alcanzado'})}});
 (clockRows||[]).slice(0,8).forEach(r=>events.push({time:r.created_at,icon:String(r.type).toLowerCase()==='entrada'?'ðŸ‘¤':'ðŸšª',text:`${r.employee_name} ${String(r.type).toLowerCase()==='entrada'?'entra':'sale'}`}));
...
function Gestoria(){
 ...
 async function load(){
  try{setLoading(true);setProgress('Preparando gestorÃ­a...');setData(await loadSalesRangeGestoria(range.from,range.to,setProgress));}
  catch(e){alert('Error cargando gestorÃ­a: '+(e.message||String(e)));}
 ...
 const exportResumen=()=>{const rows=[['Informe gestorÃ­a',range.label],['Rango',range.from,range.to],['Tickets incluidos',fiscalRows.length],[],['Concepto','Importe'],['Ventas totales',sum.total.toFixed(2)],['Tickets',sum.tickets],['Ticket medio',sum.ticket_medio.toFixed(2)],['Efectivo',sum.efectivo.toFixed(2)],['Tarjeta',sum.tarjeta.toFixed(2)],['Cheque/Otros',sum.cheque.toFixed(2)],[],['IVA','Base imponible','Cuota IVA','Total'],...iva.map(r=>[r.iva+'%',r.base.toFixed(2),r.cuota.toFixed(2),r.total.toFixed(2)])];downloadFile(`Gestoria_RESUMEN_${range.label.replaceAll(' ','_')}.csv`,asCSV(rows),'text/csv;charset=utf-8')};
 const exportTickets=()=>{const rows=[['NÂº ticket','CAB_ID','Fecha','Hora','Forma pago','Mesa','Estado','Base 0%','IVA 0%','Total 0%','Base 10%','IVA 10%','Total 10%','Base 21%','IVA 21%','Total 21%','Base otros','IVA otros','Total otros','Base total','IVA total','Total ticket','Efectivo','Tarjeta','Cheque/Otros'],...fiscalRows.map(r=>[r.numdoc,r.cab_id,r.fecha,r.hora,r.forma,r.mesa,r.estado,r.base0.toFixed(2),r.iva0.toFixed(2),r.total0.toFixed(2),r.base10.toFixed(2),r.iva10.toFixed(2),r.total10.toFixed(2),r.base21.toFixed(2),r.iva21.toFixed(2),r.total21.toFixed(2),r.baseOtros.toFixed(2),r.ivaOtros.toFixed(2),r.totalOtros.toFixed(2),r.baseTotal.toFixed(2),r.ivaTotal.toFixed(2),r.total.toFixed(2),r.efectivo.toFixed(2),r.tarjeta.toFixed(2),r.otros.toFixed(2)])];downloadFile(`Gestoria_TICKETS_${range.label.replaceAll(' ','_')}.csv`,asCSV(rows),'text/csv;charset=utf-8')};
 return <div><div className="card hero"><div><h2>ðŸ“ GestorÃ­a contable PRO</h2><p>Informe fiscal completo sin lÃ­mite de 1000 tickets. Incluye listado de tickets con IVA y forma de pago.</p></div><div className="row controls"><select value={type} onChange={e=>{setType(e.target.value);setPeriod(1)}}><option value="trimestre">Trimestre</option><option value="mes">Mes</option></select><input type="number" value={year} onChange={e=>setYear(e.target.value)}/>{type==='trimestre'?<select value={period} onChange={e=>setPeriod(e.target.value)}><option value="1">1T</option><option value="2">2T</option><option value="3">3T</option><option value="4">4T</option></select>:<select value={period} onChange={e=>setPeriod(e.target.value)}>{Array.from({length:12},(_,i)=><option value={i+1} key={i}>{String(i+1).padStart(2,'0')}</option>)}</select>}<button onClick={load}>{loading?'Cargando...':'Actualizar'}</button></div></div>{loading&&<div className="card"><h2>Procesando informe...</h2><p>{progress}</p></div>}<SalesCards daily={sum} sync={data.sync}/><div className="grid"><div className="card"><h2>Resumen fiscal Â· {range.label}</h2><table><tbody><tr><td>Ventas totales</td><td>{money(sum.total)}</td></tr><tr><td>Tickets reales incluidos</td><td>{sum.tickets}</td></tr><tr><td>Ticket medio</td><td>{money(sum.ticket_medio)}</td></tr><tr><td>Efectivo</td><td>{money(sum.efectivo)}</td></tr><tr><td>Tarjeta</td><td>{money(sum.tarjeta)}</td></tr><tr><td>Cheque/Otros</td><td>{money(sum.cheque)}</td></tr></tbody></table><div className="row"><button onClick={exportResumen}>Exportar resumen CSV</button><button onClick={exportTickets}>Exportar listado tickets CSV</button><button onClick={()=>window.print()}>Imprimir / PDF</button></div><p className="mutedText">El listado de tickets se ordena por nÃºmero de ticket e incluye base imponible, IVA, total y forma de pago.</p></div><div className="card"><h2>Desglose IVA</h2><table><thead><tr><th>IVA</th><th>Base imponible</th><th>Cuota IVA</th><th>Total</th></tr></thead><tbody>{iva.map(r=><tr key={r.iva}><td>{r.iva}%</td><td>{money(r.base)}</td><td>{money(r.cuota)}</td><td>{money(r.total)}</td></tr>)}</tbody></table>{iva.length===0&&<p>No hay lÃ­neas de IVA para este periodo.</p>}</div></div><div className="card"><h2>Listado de tickets incluidos</h2><p>{fiscalRows.length.toLocaleString('es-ES')} tickets cargados en este periodo.</p><table><thead><tr><th>NÂº ticket</th><th>Fecha</th><th>Hora</th><th>Forma pago</th><th>Base</th><th>IVA</th><th>Total</th></tr></thead><tbody>{fiscalRows.slice(0,200).map(r=><tr key={r.cab_id}><td>{r.numdoc||r.cab_id}</td><td>{r.fecha}</td><td>{r.hora}</td><td>{r.forma}</td><td>{money(r.baseTotal)}</td><td>{money(r.ivaTotal)}</td><td>{money(r.total)}</td></tr>)}</tbody></table>{fiscalRows.length>200&&<p className="mutedText">Mostrando 200 primeros en pantalla. La exportaciÃ³n incluye todos.</p>}</div></div>
}
