...
function Profitability(){
 ...
 const[newSupplier,setNewSupplier]=useState('');const[file,setFile]=useState(null);
 const[invForm,setInvForm]=useState({supplier_id:'',invoice_number:'',invoice_date:today(),subtotal:'',tax_total:'',total:''});
 const[lineForm,setLineForm]=useState({product_name:'',supplier_sku:'',article_code:'',quantity:1,pack_units:1,unit:'ud',net_total:'',tax_rate:10,sale_price:''});
 useEffect(()=>{loadAll()},[]);
 async function loadAll(){
  ...
  const [sRes,iRes,lRes]=await Promise.all([
   ...
   supabase.from('purchase_invoices').select('*,purchase_suppliers(name)').order('invoice_date',{ascending:false}).limit(500),
 ...
 async function createSupplier(){const name=newSupplier.trim();if(!name)return;const{data,error}=await supabase.from('purchase_suppliers').insert({name,active:true}).select().single();if(error)return alert(error.message);setNewSupplier('');setSuppliers(v=>[...v,data].sort((a,b)=>a.name.localeCompare(b.name)));setInvForm(v=>({...v,supplier_id:data.id}))}
 async function createInvoice(){
  if(!invForm.supplier_id)return alert('Selecciona un proveedor');
  let file_path=null,file_name=null,file_type=null;
  if(file){const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');file_path=`${new Date().getFullYear()}/${Date.now()}_${safe}`;const up=await supabase.storage.from('purchase-invoices').upload(file_path,file,{contentType:file.type,upsert:false});if(up.error)return alert('No se pudo subir la factura: '+up.error.message);file_name=file.name;file_type=file.type;}
  const row={supplier_id:invForm.supplier_id,invoice_number:invForm.invoice_number||null,invoice_date:invForm.invoice_date,subtotal:Number(invForm.subtotal||0),tax_total:Number(invForm.tax_total||0),total:Number(invForm.total||0),file_path,file_name,file_type,status:'pending_review',source:file?'upload':'manual'};
  const{data,error}=await supabase.from('purchase_invoices').insert(row).select().single();if(error)return alert(error.message);
  setInvForm({supplier_id:invForm.supplier_id,invoice_number:'',invoice_date:today(),subtotal:'',tax_total:'',total:''});setFile(null);setSelectedId(data.id);await loadAll();setT('facturas');
 }
 async function addLine(){
  ...
  const row={invoice_id:selectedId,product_name:lineForm.product_name.trim(),supplier_sku:lineForm.supplier_sku||null,article_code:lineForm.article_code||null,quantity:Number(lineForm.quantity||0),pack_units:Number(lineForm.pack_units||1),unit:lineForm.unit||'ud',net_total:Number(lineForm.net_total||0),tax_rate:Number(lineForm.tax_rate||0),unit_cost,sale_price:Number(lineForm.sale_price||0)};
  const{error}=await supabase.from('purchase_invoice_items').insert(row);if(error)return alert(error.message);
  setLineForm({product_name:'',supplier_sku:'',article_code:'',quantity:1,pack_units:1,unit:'ud',net_total:'',tax_rate:10,sale_price:''});await loadAll();
 }
 async function removeLine(id){if(!confirm('¿Eliminar esta línea?'))return;const{error}=await supabase.from('purchase_invoice_items').delete().eq('id',id);if(error)alert(error.message);else loadAll()}
 async function markReviewed(id){const{error}=await supabase.from('purchase_invoices').update({status:'reviewed',reviewed_at:new Date().toISOString()}).eq('id',id);if(error)alert(error.message);else loadAll()}
 async function openFile(inv){if(!inv.file_path)return alert('Esta factura no tiene archivo');const{data,error}=await supabase.storage.from('purchase-invoices').createSignedUrl(inv.file_path,300);if(error)return alert(error.message);window.open(data.signedUrl,'_blank','noopener,noreferrer')}
 ...
 const priceAlerts=histories.filter(x=>x.change!==null&&x.change>=8).sort((a,b)=>b.change-a.change);
 function exportCosts(){const rows=[['Producto','Proveedor','Fecha','Coste unidad','PVP','Margen %','Código NUMIER'],...latestByProduct.map(x=>[x.product_name,x.purchase_invoices?.purchase_suppliers?.name||'',x.purchase_invoices?.invoice_date||'',Number(x.unit_cost||0).toFixed(4),Number(x.sale_price||0).toFixed(2),marginPct(x.sale_price,x.unit_cost).toFixed(2),x.article_code||''])];downloadFile(`costes_${today()}.csv`,asCSV(rows),'text/csv;charset=utf-8')}
 if(loading)return <div className="card"><h2>📦 Cargando Costes y Rentabilidad...</h2></div>;
 return <div className="profitPage">
  <div className="card hero profitHero"><div><h2>📦 Costes y Rentabilidad</h2><p>Control de compras, proveedores, evolución de precios y margen real por artículo.</p></div><div className="row"><button onClick={loadAll}>Actualizar</button><button onClick={exportCosts}>Exportar costes</button></div></div>
  {error&&<div className="warnBox"><b>Configuración pendiente:</b> {error}. Ejecuta el SQL incluido en la RC 3.6.0.</div>}
  <div className="profitTabs">{[['resumen','Resumen'],['facturas','Facturas'],['costes','Costes'],['margenes','Márgenes'],['proveedores','Proveedores']].map(([id,label])=><button key={id} className={t===id?'active':''} onClick={()=>setT(id)}>{label}</button>)}</div>
  {t==='resumen'&&<><div className="grid profitKpis"><div className="card kpi"><span>Compras este mes</span><b>{money(monthPurchases)}</b></div><div className="card kpi"><span>Facturas</span><b>{monthInvoices.length}</b></div><div className="card kpi"><span>Artículos con coste</span><b>{latestByProduct.length}</b></div><div className="card kpi"><span>Margen medio configurado</span><b className={avgMargin>=60?'ok':avgMargin>=40?'info':'bad'}>{avgMargin.toFixed(1)}%</b></div></div><div className="grid"><div className="card"><h2>⚠️ Subidas de precio</h2>{priceAlerts.length?priceAlerts.slice(0,10).map(x=><div className="profitAlert" key={x.key}><div><b>{x.name}</b><small>{x.latest.purchase_invoices?.purchase_suppliers?.name||'Proveedor'} · {x.latest.purchase_invoices?.invoice_date}</small></div><strong>+{x.change.toFixed(1)}%</strong></div>):<p className="positiveText">No se detectan subidas superiores al 8%.</p>}</div><div className="card"><h2>💡 Recomendaciones</h2>{priceAlerts[0]?<p className="infoText">Revisa <b>{priceAlerts[0].name}</b>: su coste unitario ha subido un {priceAlerts[0].change.toFixed(1)}% respecto a la compra anterior.</p>:<p className="positiveText">Carga varias facturas para empezar a comparar precios automáticamente.</p>}{marginRows[0]&&marginRows[0].margin<40&&<p className="criticalText">El margen de <b>{marginRows[0].product_name}</b> es solo del {marginRows[0].margin.toFixed(1)}%.</p>}<p className="mutedText">La rentabilidad será más precisa cuando vincules cada compra con su código NUMIER y su PVP.</p></div></div></>}
  {t==='facturas'&&<div className="profitInvoices"><div className="card"><h2>Subir o registrar factura</h2><div className="formGrid"><label>Proveedor<select value={invForm.supplier_id} onChange={e=>setInvForm({...invForm,supplier_id:e.target.value})}><option value="">Seleccionar...</option>{suppliers.filter(x=>x.active!==false).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>Número<input value={invForm.invoice_number} onChange={e=>setInvForm({...invForm,invoice_number:e.target.value})}/></label><label>Fecha<input type="date" value={invForm.invoice_date} onChange={e=>setInvForm({...invForm,invoice_date:e.target.value})}/></label><label>Subtotal<input type="number" step="0.01" value={invForm.subtotal} onChange={e=>setInvForm({...invForm,subtotal:e.target.value})}/></label><label>IVA<input type="number" step="0.01" value={invForm.tax_total} onChange={e=>setInvForm({...invForm,tax_total:e.target.value})}/></label><label>Total<input type="number" step="0.01" value={invForm.total} onChange={e=>setInvForm({...invForm,total:e.target.value})}/></label><label className="wideField">Foto o PDF<input type="file" accept="image/*,.pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/></label></div><button onClick={createInvoice}>Guardar factura</button><p className="infoText">La foto/PDF queda guardada en la bandeja. Los datos se confirman antes de afectar a costes y márgenes.</p><hr/><div className="row"><input placeholder="Nuevo proveedor" value={newSupplier} onChange={e=>setNewSupplier(e.target.value)}/><button onClick={createSupplier}>Crear proveedor</button></div></div><div className="card"><h2>Facturas registradas</h2><div className="invoiceList">{invoices.map(inv=><button key={inv.id} className={selectedId===inv.id?'selected':''} onClick={()=>setSelectedId(inv.id)}><span><b>{inv.purchase_suppliers?.name||'Proveedor'}</b><small>{inv.invoice_date} · {inv.invoice_number||'Sin número'}</small></span><strong>{money(inv.total)}</strong><em className={inv.status==='reviewed'?'statusOk':'statusPending'}>{inv.status==='reviewed'?'Revisada':'Pendiente'}</em></button>)}</div></div>{selected&&<div className="card wide"><div className="row between"><div><h2>Detalle de factura</h2><p>{selected.purchase_suppliers?.name} · {selected.invoice_date} · {money(selected.total)}</p></div><div className="row">{selected.file_path&&<button onClick={()=>openFile(selected)}>Ver archivo</button>}<button onClick={()=>markReviewed(selected.id)}>Marcar revisada</button></div></div><div className="formGrid lineForm"><label>Producto<input value={lineForm.product_name} onChange={e=>setLineForm({...lineForm,product_name:e.target.value})}/></label><label>SKU proveedor<input value={lineForm.supplier_sku} onChange={e=>setLineForm({...lineForm,supplier_sku:e.target.value})}/></label><label>Código NUMIER<input value={lineForm.article_code} onChange={e=>setLineForm({...lineForm,article_code:e.target.value})}/></label><label>Bultos / cantidad<input type="number" step="0.001" value={lineForm.quantity} onChange={e=>setLineForm({...lineForm,quantity:e.target.value})}/></label><label>Unidades por bulto<input type="number" step="0.001" value={lineForm.pack_units} onChange={e=>setLineForm({...lineForm,pack_units:e.target.value})}/></label><label>Unidad<input value={lineForm.unit} onChange={e=>setLineForm({...lineForm,unit:e.target.value})}/></label><label>Neto de línea<input type="number" step="0.01" value={lineForm.net_total} onChange={e=>setLineForm({...lineForm,net_total:e.target.value})}/></label><label>IVA %<input type="number" step="0.01" value={lineForm.tax_rate} onChange={e=>setLineForm({...lineForm,tax_rate:e.target.value})}/></label><label>PVP<input type="number" step="0.01" value={lineForm.sale_price} onChange={e=>setLineForm({...lineForm,sale_price:e.target.value})}/></label></div><p>Coste calculado por unidad: <b>{money(calcUnitCost(lineForm))}</b></p><button onClick={addLine}>Añadir artículo</button><table><thead><tr><th>Producto</th><th>Cantidad</th><th>Uds.</th><th>Neto</th><th>Coste/ud</th><th>PVP</th><th>Margen</th><th></th></tr></thead><tbody>{selectedLines.map(x=><tr key={x.id}><td><b>{x.product_name}</b><small>{x.article_code||''}</small></td><td>{x.quantity}</td><td>{x.pack_units}</td><td>{money(x.net_total)}</td><td>{money(x.unit_cost)}</td><td>{money(x.sale_price)}</td><td>{marginPct(x.sale_price,x.unit_cost).toFixed(1)}%</td><td><button className="red" onClick={()=>removeLine(x.id)}>×</button></td></tr>)}</tbody></table></div>}</div>}
  {t==='costes'&&<div className="card"><div className="row between"><div><h2>Histórico de costes</h2><p>Último precio, variación y proveedor más económico.</p></div><button onClick={exportCosts}>Exportar CSV</button></div><table><thead><tr><th>Producto</th><th>Último proveedor</th><th>Último coste</th><th>Variación</th><th>Mejor coste histórico</th><th>Proveedor</th></tr></thead><tbody>{histories.map(x=><tr key={x.key}><td><b>{x.name}</b></td><td>{x.latest.purchase_invoices?.purchase_suppliers?.name||'-'}</td><td>{money(x.latest.unit_cost)}</td><td className={x.change>5?'bad':x.change<0?'ok':''}>{x.change===null?'-':`${x.change>=0?'+':''}${x.change.toFixed(1)}%`}</td><td>{money(x.cheapest?.unit_cost)}</td><td>{x.cheapest?.purchase_invoices?.purchase_suppliers?.name||'-'}</td></tr>)}</tbody></table></div>}
  {t==='margenes'&&<div className="card"><h2>Márgenes por artículo</h2><p className="mutedText">El PVP se confirma en cada artículo. Más adelante se podrá sincronizar automáticamente con la tarifa de NUMIER.</p><table><thead><tr><th>Producto</th><th>Código NUMIER</th><th>Coste</th><th>PVP</th><th>Beneficio unitario</th><th>Margen</th><th>Estado</th></tr></thead><tbody>{marginRows.map(x=><tr key={x.id}><td><b>{x.product_name}</b></td><td>{x.article_code||'-'}</td><td>{money(x.unit_cost)}</td><td>{money(x.sale_price)}</td><td>{money(x.profit)}</td><td>{x.margin.toFixed(1)}%</td><td className={x.margin>=60?'ok':x.margin>=40?'info':'bad'}>{x.margin>=60?'Rentable':x.margin>=40?'Revisar':'Crítico'}</td></tr>)}</tbody></table>{!marginRows.length&&<p>Añade el PVP a los artículos para calcular los márgenes.</p>}</div>}
