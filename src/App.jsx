...
async function loadServiceState(){
 if(!supabase)return {open:[],status:null,error:'Supabase no configurado'};
 const cutoff=new Date(Date.now()-5*60*1000).toISOString();
 const [{data:openData,error:openError},{data:statusData}]=await Promise.all([
 ...
 return {open:openData||[],status:statusData||null,error:openError?.message||null};
}
...
function ServiceTable({def,account}){const open=!!account;const mins=minutesOpen(account?.opened_at);return <div className={'serviceTable '+(open?'open':'free')} style={{left:def.x+'%',top:def.y+'%'}} title={open?`Mesa ${def.n} · ${money(account.total)} · ${durationShort(mins)}`:`Mesa ${def.n} libre`}><div className="tableCircle">{def.n}</div><div className="tableLabel">{open?<><b>{money(account.total)}</b><span>{durationShort(mins)}</span><em>Abierta</em></>:<b>Libre</b>}</div></div>}
function ServiceZoneMap({title,tables,openByMesa}){return <div className="serviceZone"><h3>{title}</h3><div className="floorGrid">{tables.map(t=><ServiceTable key={t.n} def={t} account={openByMesa.get(t.n)}/>)}</div></div>}
function EstadoServicio(){const[state,setState]=useState({open:[],status:null,error:null});const[loading,setLoading]=useState(false);useEffect(()=>{load();const t=setInterval(load,15000);return()=>clearInterval(t)},[]);async function load(){setLoading(true);setState(await loadServiceState());setLoading(false)}const open=state.open||[];const openByMesa=new Map(open.filter(o=>Number(o.mesa_numero)<=30).map(o=>[Number(o.mesa_numero),o]));const terrace=open.filter(o=>o.zona==='terraza');const salon=open.filter(o=>o.zona==='salon');const barra=open.filter(o=>o.zona==='barra');const totalPending=open.reduce((a,o)=>a+Number(o.total||0),0);const avgMin=open.length?Math.round(open.reduce((a,o)=>a+minutesOpen(o.opened_at),0)/open.length):0;const occTerrace=Math.round((terrace.length/15)*100);const occSalon=Math.round((salon.length/8)*100);const last=state.status?.updated_at||open[0]?.last_seen_at;return <div className="servicePage"><div className="serviceHeader"><div><span className="pill">Colibrí ERP PRO 3.0.2 · Beta</span><h1>Estado del Servicio</h1><p>Restaurante en directo · mesas abiertas, zonas y pendiente de cobro.</p></div><div className="serviceLive"><b>🟢 EN DIRECTO</b><span>{last?secondsAgo(last):'sin datos'}</span><button onClick={load}>{loading?'Actualizando...':'Actualizar'}</button></div></div>{state.error&&<div className="alertBad">Error: {state.error}</div>}<div className="serviceKpis"><div><span>Ocupación total</span><b>{open.length} / 23</b><em>{Math.round((open.filter(o=>o.zona!=='barra').length/23)*100)}%</em></div><div><span>Pendiente de cobro</span><b>{money(totalPending)}</b><em>{open.length} cuentas abiertas</em></div><div><span>Terraza</span><b>{terrace.length} / 15</b><em>{occTerrace}%</em></div><div><span>Salón</span><b>{salon.length} / 8</b><em>{occSalon}%</em></div><div><span>Tiempo medio</span><b>{durationShort(avgMin)}</b><em>cuentas abiertas</em></div></div><div className="serviceLayout"><main><ServiceZoneMap title="TERRAZA (00-19)" tables={TERRACE_TABLES} openByMesa={openByMesa}/><ServiceZoneMap title="SALÓN / BARRA (20-30)" tables={SALON_TABLES} openByMesa={openByMesa}/></main><aside className="servicePanel"><div className="sideCard"><h3>Ocupación por zona</h3><p><span>Terraza</span><b>{terrace.length}/15</b><em>{occTerrace}%</em></p><p><span>Salón</span><b>{salon.length}/8</b><em>{occSalon}%</em></p><p><span>Barra</span><b>{barra.length}</b><em>cuentas</em></p></div><div className="sideCard"><h3>Cuentas abiertas</h3><p><span>Terraza</span><b>{terrace.length}</b><em>{money(terrace.reduce((a,o)=>a+Number(o.total||0),0))}</em></p><p><span>Salón</span><b>{salon.length}</b><em>{money(salon.reduce((a,o)=>a+Number(o.total||0),0))}</em></p><p><span>Barra</span><b>{barra.length}</b><em>{money(barra.reduce((a,o)=>a+Number(o.total||0),0))}</em></p><p className="total"><span>Total pendiente</span><b></b><em>{money(totalPending)}</em></p></div><div className="sideCard"><h3>Últimas aperturas</h3>{open.slice().sort((a,b)=>new Date(b.opened_at)-new Date(a.opened_at)).slice(0,5).map(o=><p key={o.cab_id}><span>{o.zona==='barra'?'Barra':`Mesa ${o.mesa_numero}`}</span><b>{money(o.total)}</b><em>{durationShort(minutesOpen(o.opened_at))}</em></p>)}{open.length===0&&<p><span>No hay cuentas abiertas</span></p>}</div><div className="sideCard legend"><h3>Leyenda</h3><p><i className="dot free"></i> Libre</p><p><i className="dot open"></i> Cuenta abierta</p><p><i className="dot warn"></i> Cuenta destacada</p></div><div className="sideCard about"><h3>Software</h3><p><b>Colibrí ERP PRO</b><br/>Versión 3.0.2 · Beta</p><p>Desarrollado por<br/><b>C.G. 21 S.L.</b><br/>C/ Amador de los Ríos, 16<br/>41003 Sevilla<br/>☎ 954 533 502</p></div></aside></div></div>}
...
function Manager(){const[tab,setTab]=useState('dashboard');const tabs=['dashboard','servicio','inteligencia','tpv','empleados','fichajes','cuadrantes','comparador','config'];return <div><header><Brand/><nav>{tabs.map(t=><button className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{t}</button>)}</nav></header><section className="page">{tab==='dashboard'&&<Dashboard/>}{tab==='servicio'&&<EstadoServicio/>}{tab==='inteligencia'&&<BusinessIntelligence/>}{tab==='empleados'&&<Employees/>}{tab==='fichajes'&&<ClockPanel/>}{tab==='cuadrantes'&&<Schedule/>}{tab==='comparador'&&<Compare/>}{tab==='tpv'&&<TPV/>}{tab==='config'&&<Settings/>}</section></div>}
