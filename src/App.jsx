...
const BAR_LAT = Number(import.meta.env.VITE_BAR_LAT || 37.3804817);
const BAR_LNG = Number(import.meta.env.VITE_BAR_LNG || -5.9864303);
const BAR_RADIUS = Number(import.meta.env.VITE_BAR_RADIUS_METERS || 75);
const BAR_QR_TOKEN = import.meta.env.VITE_BAR_QR_TOKEN || 'COLIBRI-LOCAL-CV3-2026';
...
function SettingsPanel(){return <section className="panel"><h2>Configuración</h2><p>Dirección configurada: Avenida Carlos V, Local 3, 41004 Sevilla.</p><p>Coordenadas: {BAR_LAT}, {BAR_LNG}</p><p>Radio GPS autorizado: {BAR_RADIUS} m</p><p>QR físico del bar: escanear desde el local para validar fichaje si el GPS falla.</p><img className="qrPrint" src="/qr_bar_colibri.png" alt="QR fichaje bar"/><p className="muted">Enlace QR: https://fichar.braseria-elcolibri.es/?qr={BAR_QR_TOKEN}</p><p>PIN gerente: configurado por variable de entorno.</p></section>}
...
function ClockPortal(){ const [name,setName]=useState('Sonia'), [pin,setPin]=useState(''), [note,setNote]=useState(''), [msg,setMsg]=useState(''), [busy,setBusy]=useState(false), [geo,setGeo]=useState(null);
  ...
  const qrParam = urlParams.get('qr') || (window.location.hash||'').replace('#qr=','');
  const qrValid = qrParam === BAR_QR_TOKEN;
  const getGeo=()=>new Promise((res,rej)=>navigator.geolocation?navigator.geolocation.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:15000,maximumAge:0}):rej(new Error('GPS no disponible')));
  const register=async(type, methodNote)=>{ if(!supabase) throw new Error('Supabase no configurado'); const {data,error}=await supabase.rpc('registrar_fichaje',{p_employee_name:name,p_pin:pin,p_type:type,p_note:`${methodNote} · ${note||''}`}); if(error) throw error; return data; };
  const punch=async(type)=>{setBusy(true);setMsg('Comprobando GPS...'); try{const pos=await getGeo(); const lat=pos.coords.latitude,lng=pos.coords.longitude,dist=distanceMeters(lat,lng,BAR_LAT,BAR_LNG); setGeo({lat,lng,dist}); if(dist>BAR_RADIUS && !qrValid){setMsg(`❌ Fuera del radio autorizado (${dist} m). Si estás en el bar, escanea el QR físico de fichaje y vuelve a intentarlo.`); setBusy(false); return;} const methodNote = dist<=BAR_RADIUS ? `GPS OK ${dist}m (${lat.toFixed(6)},${lng.toFixed(6)})` : `QR BAR OK + GPS fuera ${dist}m (${lat.toFixed(6)},${lng.toFixed(6)})`; const data=await register(type, methodNote); setMsg(data?.ok?`✅ Fichaje registrado: ${name} · ${type.toUpperCase()} · ${dist<=BAR_RADIUS?'GPS':'QR BAR'} · ${dist} m`:`❌ ${data?.message||'No registrado'}`);}catch(e){ if(qrValid){ try{ const data=await register(type, 'QR BAR OK · GPS no disponible'); setMsg(data?.ok?`✅ Fichaje registrado por QR del bar: ${name} · ${type.toUpperCase()}`:`❌ ${data?.message||'No registrado'}`);}catch(err){ setMsg('❌ '+(err.message||'Error al fichar')); } } else {setMsg('❌ '+(e.message||'Error al fichar')+'. Si estás en el bar, escanea el QR físico de fichaje.');}} setBusy(false); };
  return <main className="clockPage"><section className="clockCard"><img src="/logo_colibri.png"/><h1>Fichaje empleados</h1><p>Brasería El Colibrí</p>{qrValid&&<div className="qrOk"><ShieldCheck/> QR del bar validado</div>}<label>Empleado</label><select value={name} onChange={e=>setName(e.target.value)}>{employees.map(e=><option key={e.name}>{e.name}</option>)}</select><label>PIN</label><input type="password" value={pin} onChange={e=>setPin(e.target.value)} placeholder="Introduce tu PIN"/><label>Observación</label><input value={note} onChange={e=>setNote(e.target.value)} placeholder="Opcional"/><div className="gps"><MapPin/> GPS obligatorio · radio {BAR_RADIUS} m {geo&&`· última distancia ${geo.dist} m`}</div><small className="help">Si el GPS marca fuera estando dentro del bar, escanea el QR físico colocado en el local.</small><button className="entry" disabled={busy} onClick={()=>punch('entrada')}><LogIn/> Entrada</button><button className="exit" disabled={busy} onClick={()=>punch('salida')}><LogOut/> Salida</button>{msg&&<div className="message">{msg}</div>}</section></main>}
