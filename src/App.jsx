...
function UnifiedProfitability(){
 ...
 async function load(){if(!supabase)return;setLoading(true);try{const salesLoader=(mode==='anio'||mode==='treinta'||mode==='mes')?loadSalesRangeGestoria:loadSalesRange;const [{tickets,lines},costMap,real,{data:f},{data:v}]=await Promise.all([salesLoader(r.from,r.to),loadProfitabilityCostMap(),loadRealProfitability(supabase,r.from,r.to,[]),supabase.from('business_fixed_expenses').select('*').order('active',{ascending:false}).order('start_date',{ascending:false}).limit(1000),supabase.from('business_variable_expenses').select('*').order('expense_date',{ascending:false}).limit(2000)]);setState({tickets:tickets.filter(isRealSaleTicket),lines:normalizePeriodLines(tickets.filter(isRealSaleTicket),lines),costMap,real});setFixed(f||[]);setVariable(v||[])}finally{setLoading(false)}}
 ...
 const details={
  ...
  personal:{title:'Personal devengado',kind:'labor',rows:[['Origen',real.laborSource==='clock_records'?'Fichajes reales':'Cuadrante previsto'],['Horas imputadas',`${Number(real.hours||0).toFixed(2)} h`],['Coste total',money(real.laborAccrued)]],items:real.details?.labor||[]},
  fijos:{title:'Gastos fijos imputados',kind:'fixed',rows:[['Total',money(real.fixed)]],items:real.details?.fixed||[]},
  variables:{title:'Gastos variables del periodo',kind:'variable',rows:[['Total',money(real.variable)]],items:real.details?.variable||[]},
...
function StaffControl({initialTab='plantilla'}){
...
function parseCardPaymentWorkbook(buffer,fileName=''){const book=XLSX.read(buffer,{type:'array',cellDates:true});const sheet=book.Sheets[book.SheetNames[0]];const rows=XLSX.utils.sheet_to_json(sheet,{defval:null,raw:false});const seen=new Set();const duplicates=[];const payments=[];
 rows.forEach((row,index)=>{const type=String(paymentField(row,['tipo','type'])||'Venta').trim();if(type&&!/venta|sale|payment|pago/i.test(type))return;const date=parsePaymentDate(paymentField(row,['fecha','date','fecha y hora','fecha hora']));const amount=Number(String(paymentField(row,['precio (neto)','precio neto','importe neto','importe','precio (bruto)','precio bruto','amount'])??'').replace(',','.'));const id=String(paymentField(row,['id de transaccion','id transaccion','transaction id','referencia','id'])||`${fileName}-${index}`).trim();if(!date||!Number.isFinite(amount)||amount<=0)return;const item={id,date,dateISO:localISODate(date),amount:Math.round(amount*100)/100,method:String(paymentField(row,['forma de pago','metodo de pago','payment method'])||'Tarjeta'),account:String(paymentField(row,['cuenta','account'])||''),sourceRow:index+2};if(seen.has(id))duplicates.push(item);else{seen.add(id);payments.push(item)}});
 payments.sort((a,b)=>a.date-b.date);return {payments,duplicates,sheetName:book.SheetNames[0],fileName};}
...
function reconcileCardPayments(payments,tickets){
 ...
 for(const payment of payments){let best=null;for(const ticket of validTickets){if(used.has(String(ticket.cab_id)))continue;if(localISODate(ticket._date)!==payment.dateISO)continue;const diffMin=Math.abs(payment.date-ticket._date)/60000;const cardDiff=Math.abs(payment.amount-ticket._card);const totalDiff=Math.abs(payment.amount-ticket._total);let kind=null,score=-1;
  ...
  if(best&&best.score>20){used.add(String(best.ticket.cab_id));matches.push({payment,...best,status:best.kind})}else matches.push({payment,status:'payment_without_ticket'});
 }
 const ticketWithoutPayment=validTickets.filter(t=>t._card>0&&!used.has(String(t.cab_id))).map(ticket=>({ticket,status:'ticket_without_payment',expected:ticket._card}));
 const counts={correct:0,probable:0,wrong_method:0,amount_mismatch:0,payment_without_ticket:0,ticket_without_payment:0};[...matches,...ticketWithoutPayment].forEach(x=>{counts[x.status]=(counts[x.status]||0)+1});
 const realTotal=payments.reduce((a,p)=>a+p.amount,0),tpvCard=validTickets.reduce((a,t)=>a+t._card,0);return {matches,ticketWithoutPayment,counts,realTotal,tpvCard,difference:realTotal-tpvCard,tickets:validTickets};
}
function paymentStatusMeta(status){return {correct:['Correcto','ok'],probable:['Coincidencia probable','info'],wrong_method:['Forma de pago errónea','bad'],amount_mismatch:['Importe distinto','warn'],payment_without_ticket:['Cobro sin ticket','bad'],ticket_without_payment:['Ticket sin cobro','bad']}[status]||[status,'info']}
function PaymentReconciliation({initialDate}){
 const[fileInfo,setFileInfo]=useState(null),[result,setResult]=useState(null),[loading,setLoading]=useState(false),[error,setError]=useState(''),[filter,setFilter]=useState('incidencias'),[selectedTicket,setSelectedTicket]=useState(null);
 async function importFile(file){if(!file)return;setLoading(true);setError('');try{const parsed=parseCardPaymentWorkbook(await file.arrayBuffer(),file.name);if(!parsed.payments.length)throw new Error('No se han encontrado cobros válidos en el archivo.');const dates=parsed.payments.map(x=>x.dateISO).sort();const from=dates[0],to=addDays(dates[dates.length-1],1);const sales=await loadSalesRangeGestoria(from,to);const tickets=(sales.tickets||[]).filter(isRealSaleTicket);const reconciliation=reconcileCardPayments(parsed.payments,tickets);const payload={...parsed,...reconciliation,from,to:addDays(to,-1),importedAt:new Date().toISOString()};setFileInfo(payload);setResult(payload);try{localStorage.setItem('colibri47_last_reconciliation',JSON.stringify({fileName:file.name,from,payments:parsed.payments.length,realTotal:reconciliation.realTotal,tpvCard:reconciliation.tpvCard,difference:reconciliation.difference,counts:reconciliation.counts,importedAt:payload.importedAt}))}catch{}}catch(e){setError(e.message||String(e));setFileInfo(null);setResult(null)}finally{setLoading(false)}}
 function exportReport(){if(!result)return;const rows=[['Estado','Fecha cobro','Hora cobro','ID transacción','Cobro real','Ticket','Hora ticket','Pago TPV','Tarjeta esperada','Total ticket','Diferencia minutos','Observación']];result.matches.forEach(x=>{const [label]=paymentStatusMeta(x.status);rows.push([label,x.payment.dateISO,x.payment.date.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),x.payment.id,x.payment.amount,x.ticket?.numdoc||x.ticket?.cab_id||'',x.ticket?x.ticket._date.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}):'',x.ticket?formaPagoText(x.ticket):'',x.ticket?x.ticket._card:'',x.ticket?x.ticket._total:'',x.diffMin!=null?Math.round(x.diffMin):'',x.status==='wrong_method'?'El cobro de tarjeta coincide con un ticket registrado como efectivo/otro':'' ])});result.ticketWithoutPayment.forEach(x=>{const [label]=paymentStatusMeta(x.status);rows.push([label,localISODate(x.ticket._date),'','',0,x.ticket.numdoc||x.ticket.cab_id,x.ticket._date.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}),formaPagoText(x.ticket),x.expected,x.ticket._total,'','No aparece un cobro real que corresponda a la parte de tarjeta'])});
  const wb=XLSX.utils.book_new();const ws=XLSX.utils.aoa_to_sheet(rows);ws['!cols']=[{wch:24},{wch:12},{wch:12},{wch:20},{wch:14},{wch:14},{wch:12},{wch:24},{wch:16},{wch:14},{wch:16},{wch:50}];XLSX.utils.book_append_sheet(wb,ws,'Conciliación');const summary=XLSX.utils.aoa_to_sheet([['COLIBRÍ · CONCILIACIÓN DE COBROS'],['Archivo',result.fileName],['Periodo',result.from,result.to],['Cobros reales',result.payments.length,result.realTotal],['Tarjeta según TPV','',result.tpvCard],['Diferencia','',result.difference],[],['Estado','Operaciones'],...Object.entries(result.counts).map(([k,v])=>[paymentStatusMeta(k)[0],v])]);XLSX.utils.book_append_sheet(wb,summary,'Resumen');XLSX.writeFile(wb,`COLIBRI_CONCILIACION_${result.from}_${result.to}.xlsx`);
 }
 ...
 return <div className="paymentReconciliation"><section className="card paymentImport"><div><span className="sectionEyebrow">COLIBRÍ ERP 4.7 · CONCILIACIÓN DE COBROS</span><h2>Compara el TPV con los cobros reales de tarjeta</h2><p>Sube el Excel exportado por el datáfono. El ERP cruza importe, fecha, hora y orden de operaciones para localizar formas de pago erróneas.</p></div><label className="paymentFileButton">{loading?'Procesando…':'Subir listado de tarjeta'}<input type="file" accept=".xlsx,.xls,.csv" disabled={loading} onChange={e=>importFile(e.target.files?.[0])}/></label></section>{error&&<div className="alertBad">{error}</div>}
 {result&&<><section className="paymentReconcileKpis"><article className="card"><span>Cobros reales</span><b>{money(result.realTotal)}</b><small>{result.payments.length} operaciones</small></article><article className="card"><span>Tarjeta en TPV</span><b>{money(result.tpvCard)}</b><small>{result.tickets.filter(t=>t._card>0).length} tickets</small></article><article className={`card ${Math.abs(result.difference)>.02?'dangerKpi':''}`}><span>Diferencia</span><b>{money(result.difference)}</b><small>Real menos TPV</small></article><article className="card"><span>Incidencias</span><b>{allRows.filter(x=>x.status!=='correct').length}</b><small>{result.counts.wrong_method||0} formas erróneas</small></article></section>
 <section className="card paymentResult"><div className="row between paymentResultHead"><div><h2>Resultado de la conciliación</h2><p>{result.fileName} · {fmtDate(result.from)}{result.to!==result.from?` a ${fmtDate(result.to)}`:''}</p></div><div className="row"><button className={filter==='incidencias'?'active':''} onClick={()=>setFilter('incidencias')}>Incidencias</button><button className={filter==='correctos'?'active':''} onClick={()=>setFilter('correctos')}>Correctos</button><button className={filter==='todos'?'active':''} onClick={()=>setFilter('todos')}>Todos</button><button onClick={exportReport}>Exportar Excel</button></div></div>
 <div className="paymentRows">{visible.map((x,i)=>{const [label,tone]=paymentStatusMeta(x.status),ticket=x.ticket;return <article className={`paymentMatch ${tone}`} key={`${x.payment?.id||ticket?.cab_id}-${i}`}><div className="paymentMatchStatus"><strong>{label}</strong><small>{x.payment?.id||'Operación TPV'}</small></div><div><span>Cobro real</span><b>{x.payment?money(x.payment.amount):'—'}</b><small>{x.payment?`${fmtDate(x.payment.dateISO)} · ${x.payment.date.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}`:'Sin cobro encontrado'}</small></div><div><span>Ticket TPV</span><b>{ticket?(ticket.numdoc||ticket.cab_id):'Sin ticket'}</b><small>{ticket?`${formaPagoText(ticket)} · ${money(ticket._total)}`:'No localizado'}</small></div><div><span>Tarjeta esperada</span><b>{ticket?money(ticket._card):'—'}</b><small>{x.diffMin!=null?`${Math.round(x.diffMin)} min de diferencia`:''}</small></div>{ticket&&<button onClick={()=>setSelectedTicket(ticket.cab_id)}>Ver ticket</button>}</article>})}{!visible.length&&<div className="alertOk">No hay operaciones en este filtro.</div>}</div></section>
...
function Gestoria(){
 ...
 const gestoriaProducts=periodFinancials(normalizePeriodLines(realTickets,data.lines),gestoriaCostMap);const gestoriaReal=realResult||{laborAccrued:0,fixed:0,variable:0,hours:0};const gestoriaGross=sum.total-gestoriaProducts.cost;const gestoriaOperating=gestoriaGross-gestoriaReal.laborAccrued;const gestoriaProfit=gestoriaOperating-gestoriaReal.fixed-gestoriaReal.variable;const gestoriaMargin=sum.total?gestoriaProfit/sum.total*100:0;
 ...
 function buildWorkbook(){
  ...
  gestoriaSheet(wb,'Resumen',[[`COLIBRÍ · CIERRE ${range.label}`],['Periodo',range.from,range.to],['Estado',locked?'CERRADO':'ABIERTO'],[],['Concepto','Importe'],['Ventas',sum.total],['Tickets',sum.tickets],['Ticket medio',sum.ticket_medio],['Efectivo',sum.efectivo],['Tarjeta',sum.tarjeta],['Otros',sum.cheque],['Anulados',cancelled.length],['Gastos Numier',expensesNumier.length],[],['RENDIMIENTO REAL'],['Coste productos',gestoriaProducts.cost],['Personal devengado',gestoriaReal.laborAccrued],['Gastos fijos imputados',gestoriaReal.fixed],['Gastos variables',gestoriaReal.variable],['Margen bruto',gestoriaGross],['Margen operativo',gestoriaOperating],['Beneficio real',gestoriaProfit],['Margen real (%)',gestoriaMargin]], [28,18,18]);
