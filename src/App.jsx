...
function BusinessIntelligence({view='resumen',onNavigate}){
 ...
 const header=<><div className="card hero intelligenceHero"><div><span className="sectionEyebrow">ANÁLISIS 5.1 · INTELIGENCIA DE NEGOCIO</span><h2>Interpretar para decidir</h2><p>{r.label}. Finanzas calcula; Análisis explica; IA recomienda.</p></div><div className="row controls intelligenceRange">{buttons.map(([id,label])=><button key={id} className={mode===id?'active':''} onClick={()=>setMode(id)}>{label}</button>)}<input type="date" value={date} onChange={e=>{setDate(e.target.value);setMode('fecha')}}/><button onClick={load}>{loading?'Cargando...':'Actualizar'}</button></div></div></>;
