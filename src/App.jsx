...
function Profitability(){
 ...
 const[masterItems,setMasterItems]=useState([]);const[numierCatalog,setNumierCatalog]=useState([]);const[linking,setLinking]=useState(null);const[catalogBusy,setCatalogBusy]=useState(false);
 ...
 async function loadAll(showLoading=true){
  ...
  const [sRes,iRes,lRes,mRes,nRes,tlRes]=await Promise.all([
  ...
  const e=sRes.error||iRes.error||lRes.error||mRes.error||nRes.error;if(e)setError(e.message);
  ...
  setSuppliers(sRes.data||[]);setInvoices(iRes.data||[]);setItems(lRes.data||[]);setMasterItems(mRes.data||[]);setNumierCatalog(catalog);setLoading(false);
 }
 ...
 async function ensureMasterForLine(line){
  const normalized=normalizeProductName(line.product_name);if(!normalized)return null;
  let master=masterItems.find(x=>x.normalized_name===normalized);
  if(!master){const usage=['Materia prima'].includes(line.category)?'recipe':['Bebidas'].includes(line.category)?'sale':['Consumibles de servicio','Limpieza e higiene'].includes(line.category)?'consumable':['Menaje','Utensilios y pequeño equipamiento'].includes(line.category)?'asset':['Energía y suministros','Mantenimiento y reparación','Otros gastos'].includes(line.category)?'expense':'purchase';const{data,error}=await supabase.from('purchase_master_items').insert({name:line.product_name,normalized_name:normalized,category:line.category||'Otros gastos',sub_category:line.sub_category||null,base_unit:line.unit||'ud',usage_type:usage,active:true}).select().single();if(error&&error.code!=='23505')throw error;if(data){master=data;setMasterItems(v=>[...v,data])}else{const{data:found}=await supabase.from('purchase_master_items').select('*').eq('normalized_name',normalized).single();master=found}}
  if(master){await supabase.from('purchase_invoice_items').update({master_item_id:master.id}).eq('id',line.id);if(line.purchase_invoices?.supplier_id){await supabase.from('purchase_item_aliases').upsert({master_item_id:master.id,supplier_id:line.purchase_invoices.supplier_id,alias:line.product_name,normalized_alias:normalized},{onConflict:'supplier_id,normalized_alias'})}}
  return master;
 }
 async function consolidateCatalog(){setCatalogBusy(true);try{for(const line of items.filter(x=>x.review_status==='confirmed'&&!x.master_item_id))await ensureMasterForLine(line);await loadAll(false);alert('Catálogo de compras consolidado. Ya puedes vincular artículos con NUMIER.')}catch(e){alert('No se pudo consolidar: '+e.message)}finally{setCatalogBusy(false)}}
 async function markReviewed(inv){const lines=items.filter(x=>x.invoice_id===inv.id);if(!inv.supplier_id)return alert('Primero confirma o selecciona el proveedor');if(lines.some(x=>x.review_status!=='confirmed'))return alert('Hay artículos nuevos pendientes de clasificar');setCatalogBusy(true);try{for(const line of lines)await ensureMasterForLine(line);const{error}=await supabase.from('purchase_invoices').update({status:'reviewed',reviewed_at:new Date().toISOString(),processing_step:'Factura validada',processing_progress:100}).eq('id',inv.id);if(error)throw error;await loadAll(false)}catch(e){alert(e.message)}finally{setCatalogBusy(false)}}
 async function saveMasterLink(master,patch){const clean={...patch,updated_at:new Date().toISOString()};if(clean.numier_article_code==='')clean.numier_article_code=null;if(clean.numier_article_name==='')clean.numier_article_name=null;const{error}=await supabase.from('purchase_master_items').update(clean).eq('id',master.id);if(error)return alert(error.message);setLinking(null);await loadAll(false)}
 ...
 const latestByProduct=useMemo(()=>{const map=new Map();[...items].filter(x=>x.review_status==='confirmed'||!x.review_status).sort((a,b)=>String(b.purchase_invoices?.invoice_date||'').localeCompare(String(a.purchase_invoices?.invoice_date||''))).forEach(x=>{const k=x.master_item_id||normalizeProductName(x.product_name);if(k&&!map.has(k))map.set(k,x)});return [...map.values()]},[items]);
 const histories=useMemo(()=>{const map=new Map();items.filter(x=>x.review_status==='confirmed'||!x.review_status).forEach(x=>{const k=x.master_item_id||normalizeProductName(x.product_name);if(!map.has(k))map.set(k,[]);map.get(k).push(x)});return [...map.entries()].map(([key,rows])=>{rows.sort((a,b)=>String(b.purchase_invoices?.invoice_date||'').localeCompare(String(a.purchase_invoices?.invoice_date||'')));const latest=rows[0],previous=rows[1];const master=masterItems.find(m=>m.id===latest.master_item_id)||masterItems.find(m=>m.normalized_name===normalizeProductName(latest.product_name));const change=previous?.unit_cost?((Number(latest.unit_cost)-Number(previous.unit_cost))/Number(previous.unit_cost))*100:null;const cheapest=rows.reduce((m,r)=>!m||Number(r.unit_cost)<Number(m.unit_cost)?r:m,null);return{key,name:master?.name||latest.product_name,master,latest,previous,change,cheapest,rows}}).sort((a,b)=>a.name.localeCompare(b.name))},[items,masterItems]);
 const priceAlerts=histories.filter(x=>x.change!==null&&x.change>8);
 function numierForMaster(m){return numierCatalog.find(a=>String(a.article_code)===String(m?.numier_article_code))||null}
 const marginRows=histories.map(h=>{const m=h.master;if(!m)return null;const n=numierForMaster(m);const sale=Number(m.sale_price||n?.sale_price||0);const cost=Number(h.latest.unit_cost||0);if(!(sale>0)||!['sale','recipe'].includes(m.usage_type))return null;return{id:m.id,product_name:m.name,category:m.category,unit_cost:cost,sale_price:sale,profit:sale-cost,margin:marginPct(sale,cost),numier:n}}).filter(Boolean).sort((a,b)=>a.margin-b.margin);
 const avgMargin=marginRows.length?marginRows.reduce((a,x)=>a+x.margin,0)/marginRows.length:0;
 const unlinkedCount=histories.filter(h=>h.master&&!h.master.numier_article_code&&['sale','recipe'].includes(h.master.usage_type)).length;
 function exportCosts(){const rows=[['Producto','Categoría','Tipo','Proveedor','Fecha','Coste unidad','Artículo NUMIER','PVP','Margen %'],...histories.map(h=>{const m=h.master||{};const n=numierForMaster(m);const p=Number(m.sale_price||n?.sale_price||0);return[h.name,m.category||h.latest.category||'',m.usage_type||'',h.latest.purchase_invoices?.purchase_suppliers?.name||'',h.latest.purchase_invoices?.invoice_date||'',Number(h.latest.unit_cost||0).toFixed(4),n?.article_name||'',p?p.toFixed(2):'',p?marginPct(p,h.latest.unit_cost).toFixed(2):'']})];downloadFile(`costes_${today()}.csv`,asCSV(rows),'text/csv;charset=utf-8')}
 ...
 return <div className="profitabilityModule">
  ...
  {t==='costes'&&<div className="card"><div className="row between"><div><h2>Catálogo de costes y vinculación</h2><p>Vincula solo los productos que vendes o utilizas en recetas. Limpieza, menaje y gastos siguen controlados sin PVP.</p></div><div className="row"><button disabled={catalogBusy} onClick={consolidateCatalog}>{catalogBusy?'Consolidando...':'Sincronizar catálogo'}</button><button onClick={exportCosts}>Exportar CSV</button></div></div>{unlinkedCount>0&&<div className="warnBox">Hay {unlinkedCount} artículos vendibles pendientes de vincular con NUMIER.</div>}<div className="costCatalog">{histories.map(h=>{const m=h.master;const n=numierForMaster(m);return <article className="costItemCard" key={h.key}><div><b>{h.name}</b><small>{m?.category||h.latest.category||'-'} · {h.latest.purchase_invoices?.purchase_suppliers?.name||'-'}</small><strong>{money(h.latest.unit_cost)} / {m?.base_unit||h.latest.unit||'ud'}</strong><small className={h.change>5?'bad':h.change<0?'ok':''}>{h.change===null?'Primer precio':`${h.change>=0?'+':''}${h.change.toFixed(1)}% frente al anterior`}</small></div><div className="costLinkState">{!m?<span className="statusWarning">Pendiente de consolidar</span>:m.numier_article_code?<><span className="statusOk">Vinculado</span><b>{n?.article_name||m.numier_article_name||m.numier_article_code}</b>{Number(m.sale_price||n?.sale_price)>0&&<small>PVP {money(m.sale_price||n.sale_price)}</small>}</>:['expense','consumable','asset'].includes(m.usage_type)?<><span className="statusInfo">No vendible</span><small>Controlado como coste</small></>:<span className="statusPending">Sin vincular</span>}{m&&<button onClick={()=>setLinking({master:m,search:m.name,code:m.numier_article_code||'',manualPrice:m.sale_price||'',usage:m.usage_type||'purchase'})}>{m.numier_article_code?'Editar':'Vincular / clasificar'}</button>}</div></article>})}</div></div>}
  {t==='margenes'&&<div className="card"><div className="row between"><div><h2>Márgenes por artículo</h2><p>Coste real de factura frente al PVP de NUMIER o PVP manual.</p></div>{marginRows.length>0&&<b className="bigKpi">Margen medio {avgMargin.toFixed(1)}%</b>}</div><table><thead><tr><th>Producto</th><th>Artículo NUMIER</th><th>Coste</th><th>PVP</th><th>Beneficio</th><th>Margen</th><th>Estado</th></tr></thead><tbody>{marginRows.map(x=><tr key={x.id}><td><b>{x.product_name}</b><small>{x.category||'-'}</small></td><td>{x.numier?.article_name||'-'}</td><td>{money(x.unit_cost)}</td><td>{money(x.sale_price)}</td><td>{money(x.profit)}</td><td>{x.margin.toFixed(1)}%</td><td className={x.margin>=60?'ok':x.margin>=40?'info':'bad'}>{x.margin>=60?'Rentable':x.margin>=40?'Revisar':'Crítico'}</td></tr>)}</tbody></table>{!marginRows.length&&<div className="emptyState"><b>Aún no hay productos vendibles vinculados.</b><p>Ve a Costes, pulsa “Sincronizar catálogo” y después “Vincular / clasificar” en agua, bebidas, snacks o materias primas.</p><button onClick={()=>setT('costes')}>Ir a vincular artículos</button></div>}</div>}
  {linking&&<div className="modal" onClick={()=>setLinking(null)}><div className="card linkModal" onClick={e=>e.stopPropagation()}><div className="row between"><div><h2>Vincular artículo</h2><p>{linking.master.name}</p></div><button className="red" onClick={()=>setLinking(null)}>Cerrar</button></div><label>Uso del artículo<select value={linking.usage} onChange={e=>setLinking({...linking,usage:e.target.value})}><option value="sale">Se vende directamente</option><option value="recipe">Materia prima para recetas/escandallos</option><option value="consumable">Consumible del negocio</option><option value="asset">Menaje o equipamiento</option><option value="expense">Gasto / suministro</option><option value="purchase">Pendiente de decidir</option></select></label>{['sale','recipe'].includes(linking.usage)&&<><label>Buscar en artículos NUMIER<input value={linking.search} onChange={e=>setLinking({...linking,search:e.target.value})} placeholder="Ej. AGUA 50 CL"/></label><div className="numierMatches">{numierCatalog.filter(a=>normalizeProductName(a.article_name).includes(normalizeProductName(linking.search))||normalizeProductName(linking.search).split(' ').filter(Boolean).some(w=>normalizeProductName(a.article_name).includes(w))).slice(0,12).map(a=><button key={a.article_code} className={String(linking.code)===String(a.article_code)?'selected':''} onClick={()=>setLinking({...linking,code:a.article_code,search:a.article_name,manualPrice:a.sale_price||linking.manualPrice})}><b>{a.article_name}</b><small>{a.article_code} {a.sale_price>0?`· PVP ${money(a.sale_price)}`:''}</small></button>)}</div><label>PVP manual (solo si NUMIER aún no aporta precio)<input type="number" step="0.01" value={linking.manualPrice} onChange={e=>setLinking({...linking,manualPrice:e.target.value})}/></label></>}<div className="row"><button onClick={()=>{const a=numierCatalog.find(x=>String(x.article_code)===String(linking.code));saveMasterLink(linking.master,{usage_type:linking.usage,numier_article_code:['sale','recipe'].includes(linking.usage)?linking.code:null,numier_article_name:a?.article_name||null,sale_price:Number(linking.manualPrice||a?.sale_price||0),sale_price_source:a?.sale_price>0?'numier':'manual'})}}>Guardar clasificación</button>{linking.master.numier_article_code&&<button className="red" onClick={()=>saveMasterLink(linking.master,{numier_article_code:null,numier_article_name:null,sale_price:0,sale_price_source:null})}>Desvincular</button>}</div></div></div>}
