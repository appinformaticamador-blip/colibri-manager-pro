...
function Dashboard(){const[rows,setRows]=useState([]);const[sales,setSales]=useState({daily:null,tickets:[],sync:null});const[date,setDate]=useState(today());useEffect(()=>{load();const t=setInterval(load,15000);return()=>clearInterval(t)},[date]);async function load(){if(!supabase)return;const [{data:clockData},salesData]=await Promise.all([supabase.from('clock_records').select('*').order('created_at',{ascending:false}).limit(200),loadSalesForDate(date)]);setRows(clockData||[]);setSales(salesData)}const latest=new Map();rows.forEach(r=>{const k=r.employee_id||r.employee_name;if(!latest.has(k))latest.set(k,r)});const working=[...latest.values()].filter(r=>String(r.type).toLowerCase()==='entrada');return <div><div className="card hero"><div><h2>Dashboard LIVE</h2><p>{fmtDate(date)}</p></div><div className="row controls"><button onClick={()=>setDate(today())}>Hoy</button><button onClick={()=>setDate(addDays(today(),-1))}>Ayer</button><input type="date" value={date} onChange={e=>setDate(e.target.value)}/><button onClick={load}>Actualizar</button></div></div><SyncStatusCard/><SalesCards daily={sales.daily} sync={sales.sync}/><div className="grid"><div className="card"><h2>Trabajando ahora</h2><div className="big">{working.length}</div>{working.map(w=><p key={w.id}>🟢 {w.employee_name}</p>)}</div><div className="card"><h2>Ventas por hora</h2><SalesByHour tickets={sales.tickets}/></div><div className="card"><h2>Últimos fichajes</h2>{rows.slice(0,6).map(r=><p key={r.id}>{String(r.type).toLowerCase()==='entrada'?'🟢':'🔴'} {r.employee_name} · {new Date(r.created_at).toLocaleString()}</p>)}</div><div className="card"><h2>Estado del sistema</h2><p>✅ Supabase conectado</p><p>✅ Fichajes activo</p><p>{sales.sync?'✅ NUMIER sincronizando':'⚠️ Sin registro NUMIER'}</p><p>📍 GPS/QR activo</p></div></div></div>}
