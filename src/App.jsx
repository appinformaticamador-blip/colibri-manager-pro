...
import {buildNumierCatalog,calculateMarginValues,mergeResolvedNumierPrice,numierPriceText,resolveLinkSalePrice} from './modules/profitability/numierPricing';
...
function Profitability(){
 ...
 const directMarginRows=links.map(l=>{const m=masterItems.find(x=>String(x.id)===String(l.master_item_id));const cost=Number(latestCostMap.get(String(l.master_item_id))||0)*Number(l.quantity_factor||1);const article=numierCatalog.find(a=>String(a.article_code)===String(l.numier_article_code));const values=calculateMarginValues(cost,resolveLinkSalePrice(article,l.sale_price));return{id:'l'+l.id,product_name:m?.name||'-',article_name:l.numier_article_name,...values,type:'Directo'}}).filter(x=>x.sale>0);
 const recipeMarginRows=recipes.map(r=>{const cost=recipeCost(r),article=numierCatalog.find(a=>String(a.article_code)===String(r.numier_article_code));const values=calculateMarginValues(cost,resolveLinkSalePrice(article,r.sale_price));return{id:'r'+r.id,product_name:r.name,article_name:r.numier_article_name,...values,type:'Escandallo'}}).filter(x=>x.sale>0);
