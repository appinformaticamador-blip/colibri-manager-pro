...
function Profitability(){
 ...
 async function loadAll(showLoading=true){
  ...
  const [sRes,iRes,lRes,mRes,nRes,tlRes,kRes,rRes,riRes]=await Promise.all([
   ...
   supabase.from('numier_articles').select('article_code,article_name,family,category_name').limit(20000),
   supabase.from('numier_ticket_lines').select('*').limit(10000),
  ...
  const e=sRes.error||iRes.error||lRes.error||mRes.error||nRes.error||kRes.error||rRes.error||riRes.error;if(e)setError(e.message);
  const linePrices=new Map();
  for(const r of tlRes.data||[]){const code=String(r.article_code??r.codigo??r.cod_articulo??r.articulo_codigo??r.det_articulo??r.codart??'').trim();const name=String(r.article_name??r.product_name??r.descripcion??r.articulo??r.nombre??r.det_descripcion??'').trim();const qty=Math.abs(Number(r.quantity??r.cantidad??r.unidades??r.det_cantidad??1))||1;const total=Number(r.total??r.importe??r.line_total??r.det_importe??r.precio_total??0);const direct=Number(r.unit_price??r.precio_unitario??r.price??r.precio??r.pvp??0);const price=direct>0?direct:(total>0?total/qty:0);if(price>0){if(code)linePrices.set('c:'+code,price);if(name)linePrices.set('n:'+normalizeProductName(name),price)}}
  const catalog=(nRes.data||[]).map(a=>({...a,sale_price:Number(linePrices.get('c:'+String(a.article_code||'').trim())||linePrices.get('n:'+normalizeProductName(a.article_name))||0)}));
 ...
 const latestCostMap=useMemo(()=>new Map(histories.filter(h=>h.master).map(h=>[String(h.master.id),Number(h.master.manual_unit_cost||h.latest.unit_cost||0)])),[histories]);
 function factor(unit){const u=normalizeProductName(unit);if(['kg','kilogramo','kilogramos'].includes(u))return{type:'mass',v:1000};if(['g','gr','gramo','gramos'].includes(u))return{type:'mass',v:1};if(['l','litro','litros'].includes(u))return{type:'volume',v:1000};if(['ml','mililitro','mililitros'].includes(u))return{type:'volume',v:1};return{type:'count',v:1}}
 function ingredientCost(ing){const m=masterItems.find(x=>String(x.id)===String(ing.master_item_id));if(!m)return 0;const base=factor(m.base_unit||'ud'),use=factor(ing.unit||m.base_unit||'ud');let qty=Number(ing.quantity||0);if(base.type===use.type)qty=qty*use.v/base.v;const raw=Number(latestCostMap.get(String(m.id))||0)*qty;const waste=Number(ing.waste_percent??m.waste_percent??0);return raw/(1-Math.min(95,Math.max(0,waste))/100)}
 function recipeCost(recipe){return recipeIngredients.filter(i=>String(i.recipe_id)===String(recipe.id)).reduce((a,i)=>a+ingredientCost(i),0)/Math.max(1,Number(recipe.yield_quantity||1))}
 const directMarginRows=links.map(l=>{const m=masterItems.find(x=>String(x.id)===String(l.master_item_id));const cost=Number(latestCostMap.get(String(l.master_item_id))||0)*Number(l.quantity_factor||1);const sale=Number(l.sale_price||numierCatalog.find(a=>String(a.article_code)===String(l.numier_article_code))?.sale_price||0);return{id:'l'+l.id,product_name:m?.name||'-',article_name:l.numier_article_name,cost,sale,profit:sale-cost,margin:marginPct(sale,cost),type:'Directo'}}).filter(x=>x.sale>0);
 const recipeMarginRows=recipes.map(r=>{const cost=recipeCost(r),sale=Number(r.sale_price||numierCatalog.find(a=>String(a.article_code)===String(r.numier_article_code))?.sale_price||0);return{id:'r'+r.id,product_name:r.name,article_name:r.numier_article_name,cost,sale,profit:sale-cost,margin:marginPct(sale,cost),type:'Escandallo'}}).filter(x=>x.sale>0);
 const marginRows=[...directMarginRows,...recipeMarginRows].sort((a,b)=>a.margin-b.margin),avgMargin=marginRows.length?marginRows.reduce((a,x)=>a+x.margin,0)/marginRows.length:0;
 ...
 async function saveMultiLink(){const m=linking.master;await supabase.from('purchase_master_items').update({usage_type:linking.usage,base_unit:linking.baseUnit||'ud',waste_percent:Number(linking.waste||0),manual_unit_cost:Number(linking.manualCost||0)||null,updated_at:new Date().toISOString()}).eq('id',m.id);await supabase.from('purchase_product_numier_links').delete().eq('master_item_id',m.id);if(['sale','recipe'].includes(linking.usage)&&linking.selected.length){const rows=linking.selected.map((code,i)=>{const a=numierCatalog.find(x=>String(x.article_code)===String(code));return{master_item_id:m.id,numier_article_code:String(code),numier_article_name:a?.article_name||String(code),sale_price:Number(linking.prices?.[code]||a?.sale_price||0),quantity_factor:Number(linking.factors?.[code]||1),is_primary:i===0}});const{error}=await supabase.from('purchase_product_numier_links').insert(rows);if(error)return alert(error.message);const first=rows[0];await supabase.from('purchase_master_items').update({numier_article_code:first.numier_article_code,numier_article_name:first.numier_article_name,sale_price:first.sale_price}).eq('id',m.id)}setLinking(null);await loadAll(false)}
 function openLink(m){const current=links.filter(l=>String(l.master_item_id)===String(m.id));setLinking({master:m,usage:m.usage_type||'purchase',search:m.name,selected:current.map(x=>String(x.numier_article_code)),prices:Object.fromEntries(current.map(x=>[String(x.numier_article_code),x.sale_price||''])),factors:Object.fromEntries(current.map(x=>[String(x.numier_article_code),x.quantity_factor||1])),baseUnit:m.base_unit||'ud',waste:m.waste_percent||0,manualCost:m.manual_unit_cost||''})}
 ...
 return <div className="profitPage"><Brand/>{error&&<div className="warnBox">{error}. Ejecuta el SQL de RC 3.8.0 si aún no lo has hecho.</div>}
  ...
  {t==='resumen'&&<><div className="grid profitKpis"><div className="kpi"><span>Compras mes</span><b>{money(monthInvoices.reduce((a,x)=>a+Number(x.total||0),0))}</b></div><div className="kpi"><span>Artículos maestros</span><b>{masterItems.length}</b></div><div className="kpi"><span>Escandallos</span><b>{recipes.length}</b></div><div className="kpi"><span>Margen medio</span><b>{avgMargin.toFixed(1)}%</b></div></div><div className="card"><h2>Centro de rentabilidad</h2><p className="infoText">Clasifica compras, vincula uno o varios artículos NUMIER y crea escandallos indicando gramos, mililitros o unidades.</p>{unlinkedCount>0&&<div className="warnBox">{unlinkedCount} artículos pendientes de vincular o clasificar.</div>}</div></>}
  ...
  {t==='escandallos'&&<div className="recipeLayout"><div className="card"><div className="row between"><div><h2>Escandallos</h2><p>Coste dinámico por gramos, mililitros o unidades.</p></div><button onClick={newRecipe}>Nuevo escandallo</button></div><div className="recipeCards">{recipes.map(r=>{const c=recipeCost(r),p=Number(r.sale_price||numierCatalog.find(a=>String(a.article_code)===String(r.numier_article_code))?.sale_price||0);return <button key={r.id} onClick={()=>editRecipe(r)}><b>{r.name}</b><small>{recipeIngredients.filter(i=>String(i.recipe_id)===String(r.id)).length} ingredientes</small><span>Coste {money(c)} · PVP {money(p)}</span><strong className={marginPct(p,c)>=60?'ok':marginPct(p,c)>=40?'info':'bad'}>{marginPct(p,c).toFixed(1)}%</strong></button>})}</div>{!recipes.length&&<div className="emptyState">Crea el primer escandallo seleccionando un producto NUMIER y sus materias primas.</div>}</div></div>}
  {t==='margenes'&&<div className="card"><div className="row between"><div><h2>Márgenes reales</h2><p>Productos directos y platos con escandallo.</p></div>{marginRows.length>0&&<b className="bigKpi">Media {avgMargin.toFixed(1)}%</b>}</div><div className="marginCards">{marginRows.map(x=><article key={x.id}><div><b>{x.product_name}</b><small>{x.article_name} · {x.type}</small></div><span>Coste <b>{money(x.cost)}</b></span><span>PVP <b>{money(x.sale)}</b></span><span>Beneficio <b>{money(x.profit)}</b></span><strong className={x.margin>=60?'ok':x.margin>=40?'info':'bad'}>{x.margin.toFixed(1)}%</strong></article>)}</div>{!marginRows.length&&<div className="emptyState"><b>No hay márgenes calculables.</b><p>Vincula productos directos o crea escandallos con PVP.</p></div>}</div>}
  ...
  {linking&&<div className="modal" onClick={()=>setLinking(null)}><div className="card linkModal linkModalPro" onClick={e=>e.stopPropagation()}><div className="row between"><div><h2>Vincular artículo</h2><p>{linking.master.name}</p></div><button className="red" onClick={()=>setLinking(null)}>Cerrar</button></div><label>Uso<select value={linking.usage} onChange={e=>setLinking({...linking,usage:e.target.value})}><option value="sale">Se vende directamente</option><option value="recipe">Materia prima / escandallos</option><option value="consumable">Consumible</option><option value="asset">Menaje/equipamiento</option><option value="expense">Gasto/suministro</option><option value="purchase">Pendiente</option></select></label><div className="linkSettings"><label>Unidad base<select value={linking.baseUnit} onChange={e=>setLinking({...linking,baseUnit:e.target.value})}><option>g</option><option>kg</option><option>ml</option><option>l</option><option>ud</option><option>caja</option><option>paquete</option></select></label><label>Merma %<input type="number" value={linking.waste} onChange={e=>setLinking({...linking,waste:e.target.value})}/></label><label>Coste manual por unidad base<input type="number" step="0.0001" value={linking.manualCost} onChange={e=>setLinking({...linking,manualCost:e.target.value})}/></label></div>{['sale','recipe'].includes(linking.usage)&&<><label>Buscar en NUMIER<input value={linking.search} onChange={e=>setLinking({...linking,search:e.target.value})}/></label><p className="infoText">Puedes marcar varios nombres o precios de venta para el mismo producto.</p><div className="numierMatches multi">{numierCatalog.filter(a=>{const q=normalizeProductName(linking.search),n=normalizeProductName(a.article_name);return !q||n.includes(q)||q.split(' ').some(w=>w&&n.includes(w))}).slice(0,30).map(a=>{const code=String(a.article_code),checked=linking.selected.includes(code);return <div className={'numierChoice '+(checked?'selected':'')} key={code}><button onClick={()=>setLinking({...linking,selected:checked?linking.selected.filter(x=>x!==code):[...linking.selected,code]})}><span className="checkBox">{checked?'✓':''}</span><b>{a.article_name}</b><small>{code} · PVP {money(a.sale_price)}</small></button>{checked&&<div className="linkDetail"><label>Factor usado<input type="number" step="0.001" value={linking.factors[code]||1} onChange={e=>setLinking({...linking,factors:{...linking.factors,[code]:e.target.value}})}/></label><label>PVP<input type="number" step="0.01" value={linking.prices[code]??a.sale_price??''} onChange={e=>setLinking({...linking,prices:{...linking.prices,[code]:e.target.value}})}/></label></div>}</div>})}</div></>}<button onClick={saveMultiLink}>Guardar clasificación y vínculos</button></div></div>}
  {editingRecipe&&<div className="modal" onClick={()=>setEditingRecipe(null)}><div className="card recipeModal" onClick={e=>e.stopPropagation()}><div className="row between"><div><h2>Escandallo</h2><p>Define cantidades estándar por producto.</p></div><button className="red" onClick={()=>setEditingRecipe(null)}>Cerrar</button></div><label>Producto NUMIER<select value={editingRecipe.numier_article_code} onChange={e=>{const a=numierCatalog.find(x=>String(x.article_code)===String(e.target.value));setEditingRecipe({...editingRecipe,numier_article_code:e.target.value,numier_article_name:a?.article_name||'',name:a?.article_name||editingRecipe.name,sale_price:a?.sale_price||editingRecipe.sale_price})}}><option value="">Seleccionar producto...</option>{numierCatalog.map(a=><option key={a.article_code} value={a.article_code}>{a.article_name}</option>)}</select></label><div className="linkSettings"><label>Nombre<input value={editingRecipe.name} onChange={e=>setEditingRecipe({...editingRecipe,name:e.target.value})}/></label><label>PVP<input type="number" value={editingRecipe.sale_price} onChange={e=>setEditingRecipe({...editingRecipe,sale_price:e.target.value})}/></label><label>Raciones resultantes<input type="number" value={editingRecipe.yield_quantity} onChange={e=>setEditingRecipe({...editingRecipe,yield_quantity:e.target.value})}/></label></div><h3>Ingredientes</h3><div className="recipeIngredients">{(editingRecipe.ingredients||[]).map((ing,i)=><div key={i}><select value={ing.master_item_id||''} onChange={e=>{const arr=[...editingRecipe.ingredients];arr[i]={...arr[i],master_item_id:e.target.value};setEditingRecipe({...editingRecipe,ingredients:arr})}}><option value="">Materia prima...</option>{masterItems.filter(m=>['recipe','sale','purchase'].includes(m.usage_type)).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select><input type="number" step="0.001" placeholder="Cantidad" value={ing.quantity||''} onChange={e=>{const arr=[...editingRecipe.ingredients];arr[i]={...arr[i],quantity:e.target.value};setEditingRecipe({...editingRecipe,ingredients:arr})}}/><select value={ing.unit||'g'} onChange={e=>{const arr=[...editingRecipe.ingredients];arr[i]={...arr[i],unit:e.target.value};setEditingRecipe({...editingRecipe,ingredients:arr})}}><option>g</option><option>kg</option><option>ml</option><option>l</option><option>ud</option></select><input type="number" placeholder="Merma %" value={ing.waste_percent??''} onChange={e=>{const arr=[...editingRecipe.ingredients];arr[i]={...arr[i],waste_percent:e.target.value};setEditingRecipe({...editingRecipe,ingredients:arr})}}/><button className="red" onClick={()=>setEditingRecipe({...editingRecipe,ingredients:editingRecipe.ingredients.filter((_,j)=>j!==i)})}>×</button></div>)}</div><button onClick={()=>setEditingRecipe({...editingRecipe,ingredients:[...(editingRecipe.ingredients||[]),{master_item_id:'',quantity:'',unit:'g',waste_percent:''}]})}>+ Añadir ingrediente</button><div className="recipePreview"><span>Coste estimado</span><b>{money((editingRecipe.ingredients||[]).reduce((a,i)=>a+ingredientCost(i),0)/Math.max(1,Number(editingRecipe.yield_quantity||1)))}</b></div><button onClick={saveRecipe}>Guardar escandallo</button></div></div>}
 </div>
}
