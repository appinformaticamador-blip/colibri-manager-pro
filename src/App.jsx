...
function AnalysisArea({section='indicadores',onNavigate}){
 const active=section==='asistente'?'asistente':'indicadores';
 return <div className="erpArea"><AreaHeader eyebrow="ANÁLISIS" title="Interpretación del negocio" copy="Aquí no se recalculan las finanzas: se interpretan tendencias, productos, franjas y decisiones."/><AreaTabs active={active} onChange={x=>onNavigate('analisis',x)} items={[[ 'indicadores','Indicadores e inteligencia','✦'],['asistente','Asistente IA','✧']]}/>{active==='indicadores'?<BusinessIntelligence/>:<BusinessAssistant/>}</div>
}

function Manager(){
 const legacyMap={dashboard:['inicio','resumen'],decisiones:['inicio','decisiones'],servicio:['operacion','directo'],tpv:['operacion','tpv'],resultado:['finanzas','resumen'],rentabilidad:['finanzas','costes'],empleados:['personal','plantilla'],fichajes:['personal','ahora'],cuadrantes:['personal','cuadrante'],comparador:['personal','comparador'],inteligencia:['analisis','indicadores'],asistente:['analisis','asistente'],gestoria:['gestoria',null],almacen:['almacen',null],config:['config',null]};
 ...
 const sectionNames={inicio:{resumen:'Resumen',decisiones:'Pendientes y decisiones'},operacion:{directo:'En directo',tpv:'Resumen TPV',control:'Control',tickets:'Tickets',conciliacion:'Conciliación',caja:'Caja',cierres:'Cierres',pulso:'Pulso IA'},finanzas:{resumen:'Resultado real',compras:'Compras',costes:'Productos y costes',facturas:'Facturas',escandallos:'Escandallos',margenes:'Márgenes',proveedores:'Proveedores'},personal:{ahora:'Ahora',plantilla:'Plantilla',puntualidad:'Puntualidad',historial:'Historial',cuadrante:'Cuadrante',comparador:'Comparador'},analisis:{indicadores:'Indicadores',asistente:'Asistente IA'}};
 ...
 return <div className="dashboard5">
  ...
  <section className="dash5Grid">
   <article className="card dash5Result"><div className="dash5Title"><div><span>RESULTADO REAL DE HOY</span><h2>Cuenta de resultados</h2></div><button onClick={()=>go('rentabilidad')}>Ver rentabilidad</button></div>
    <div className="dash5ResultRows"><p><span>Ventas</span><b>{money(summary.total)}</b></p><p><span>Coste de productos</span><b>- {money(productFin.cost)}</b></p><p><span>Margen bruto</span><b>{money(gross)}</b></p><p><span>Personal estimado ({Number(real.hours||0).toFixed(1)} h × 7 €)</span><b>- {money(real.laborAccrued)}</b></p><p><span>Gastos fijos imputados</span><b>- {money(real.fixed)}</b></p><p><span>Gastos variables ERP</span><b>- {money(real.variable)}</b></p><p><span>Gastos Numier (G)</span><b>- {money(real.numierExpenses||0)}</b></p><p className="total"><span>Beneficio real estimado</span><b className={realProfit>=0?'ok':'bad'}>{money(realProfit)}</b></p></div>
   </article>
