...
function Profitability(){
 ...
 const[masterItems,setMasterItems]=useState([]),[numierCatalog,setNumierCatalog]=useState([]),[links,setLinks]=useState([]);
 ...
 async function loadAll(showLoading=true){
  ...
  const [sRes,iRes,lRes,mRes,nRes,tlRes,kRes,rRes,riRes]=await Promise.all([
  ...
  const e=sRes.error||iRes.error||lRes.error||mRes.error||nRes.error||tlRes.error||kRes.error||rRes.error||riRes.error;if(e)setError(e.message);
  const catalog=buildNumierCatalog(nRes.data||[],tlRes.data||[]);
  setSuppliers(sRes.data||[]);setInvoices(iRes.data||[]);setItems(lRes.data||[]);setMasterItems(mRes.data||[]);setNumierCatalog(catalog);setLinks(kRes.data||[]);setRecipes(rRes.data||[]);setRecipeIngredients(riRes.data||[]);setLoading(false);
 }
 ...
 const histories=useMemo(()=>{const groups={};for(const x of items.filter(i=>i.review_status==='confirmed')){const key=String(x.master_item_id||normalizeProductName(x.product_name));(groups[key]??=[]).push(x)}return Object.entries(groups).map(([key,rows])=>{rows.sort((a,b)=>String(b.purchase_invoices?.invoice_date||'').localeCompare(String(a.purchase_invoices?.invoice_date||'')));const latest=rows[0],prev=rows[1];return{key,name:latest.product_name,latest,rows,master:masterItems.find(m=>String(m.id)===String(latest.master_item_id)),change:prev&&Number(prev.unit_cost)>0?((Number(latest.unit_cost)-Number(prev.unit_cost))/Number(prev.unit_cost))*100:null}}).sort((a,b)=>a.name.localeCompare(b.name))},[items,masterItems]);
  const marginRows=useMemo(()=>buildMarginProducts({links,masters:masterItems,recipes,ingredients:recipeIngredients,purchaseItems:items,numierCatalog}),[links,masterItems,recipes,recipeIngredients,items,numierCatalog]);
 ...
 return <div className="profitPage"><Brand/>{error&&<div className="warnBox">{error}. Ejecuta el SQL de RC 3.8.0 si aún no lo has hecho.</div>}
  ...
  <div className="profitTabs">{[['resumen','Resumen'],['facturas','Facturas IA'],['costes','Costes'],['escandallos','Escandallos'],['margenes','Márgenes'],['proveedores','Proveedores']].map(([id,label])=><button key={id} className={t===id?'active':''} onClick={()=>setT(id)}>{label}</button>)}</div>
  ...
  {t==='margenes'&&<EditableMargins supabase={supabase} links={links} masterItems={masterItems} recipes={recipes} recipeIngredients={recipeIngredients} purchaseItems={items} numierCatalog={numierCatalog} onRefresh={loadAll}/>}
...
function isoWeekBounds(){
 const now=new Date();const d=new Date(now.getFullYear(),now.getMonth(),now.getDate());const day=(d.getDay()+6)%7;d.setDate(d.getDate()-day);d.setHours(0,0,0,0);const end=new Date(d);end.setDate(end.getDate()+7);return {from:d,to:end,weekId:week()}
}
function durationMinutes(ms){return Math.max(0,Math.round(ms/60000))}
function hm(mins){const m=Math.max(0,Math.round(Number(mins||0)));return `${Math.floor(m/60)}h ${String(m%60).padStart(2,'0')}m`}
function pairEmployeeRecords(rows,nowMs=Date.now()){
 const sorted=(rows||[]).slice().sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));const sessions=[];let open=null;
 sorted.forEach(r=>{const type=String(r.type||'').toLowerCase();if(type==='entrada'){if(open)sessions.push({start:open,end:null,minutes:durationMinutes(new Date(r.created_at)-new Date(open.created_at)),incomplete:true});open=r}else if(type==='salida'&&open){sessions.push({start:open,end:r,minutes:durationMinutes(new Date(r.created_at)-new Date(open.created_at)),incomplete:false});open=null}});
 if(open)sessions.push({start:open,end:null,minutes:durationMinutes(nowMs-new Date(open.created_at).getTime()),incomplete:true});return sessions
}
function dayNameFromDate(date){return DAYS[(new Date(date).getDay()+6)%7]}
function scheduleForEmployee(scheduleData,employee){
 const out={};DAYS.forEach(d=>out[d]=[]);const src=scheduleData?.data||scheduleData||{};
 DAYS.forEach(day=>SLOTS.forEach(slot=>{const arr=src?.[day]?.[slot]||src?.[scheduleKey(week(),day,slot)]||[];if((arr||[]).some(x=>String(x?.id||x)===String(employee.id)||String(x?.name||'').toLowerCase()===String(employee.name).toLowerCase()))out[day].push(slot)}));return out
}
function EmployeePortal(){
 const[employees,setEmployees]=useState([]);const[selected,setSelected]=useState('');const[pin,setPin]=useState('');const[user,setUser]=useState(()=>{try{return JSON.parse(sessionStorage.getItem('colibriEmployee')||'null')}catch{return null}});const[tab,setTab]=useState('inicio');const[rows,setRows]=useState([]);const[schedule,setSchedule]=useState(null);const[msg,setMsg]=useState('');const[note,setNote]=useState('');const[qr,setQr]=useState(false);const[busy,setBusy]=useState(false);const[requestTime,setRequestTime]=useState('');const[requestReason,setRequestReason]=useState('');const[pinForm,setPinForm]=useState({current:'',next:'',repeat:''});const[nowTick,setNowTick]=useState(Date.now());
 useEffect(()=>{loadEmployees()},[]);useEffect(()=>{if(user)loadPortal()},[user]);useEffect(()=>{const t=setInterval(()=>setNowTick(Date.now()),30000);return()=>clearInterval(t)},[]);
 async function loadEmployees(){if(!supabase){setMsg('Supabase no configurado');return}const{data,error}=await supabase.from('employees').select('id,name,color,role,active,can_clock').eq('active',true).eq('can_clock',true).order('name');if(error)setMsg('❌ '+error.message);setEmployees(data||[])}
 async function login(){if(!selected||!pin)return setMsg('Selecciona tu nombre e introduce tu PIN.');setBusy(true);const{data,error}=await supabase.from('employees').select('id,name,color,role,active,can_clock,pin').eq('id',selected).maybeSingle();setBusy(false);if(error)return setMsg('❌ '+error.message);if(!data||String(data.pin)!==String(pin))return setMsg('❌ Nombre o PIN incorrecto.');if(!data.active||!data.can_clock)return setMsg('❌ El acceso está desactivado.');const safe={id:data.id,name:data.name,color:data.color,role:data.role};sessionStorage.setItem('colibriEmployee',JSON.stringify(safe));setUser(safe);setPin('');setMsg('')}
 function logout(){sessionStorage.removeItem('colibriEmployee');setUser(null);setRows([]);setSchedule(null);setTab('inicio');setMsg('')}
 async function loadPortal(){if(!supabase||!user)return;const b=isoWeekBounds();const[{data:r,error:er},{data:w,error:ew}]=await Promise.all([supabase.from('clock_records').select('*').or(`employee_id.eq.${user.id},employee_name.eq.${user.name}`).gte('created_at',b.from.toISOString()).lt('created_at',b.to.toISOString()).order('created_at',{ascending:true}),supabase.from('work_schedule_weeks').select('*').eq('restaurant_id','braseriacolibri').eq('week_id',b.weekId).maybeSingle()]);if(er)setMsg('❌ '+er.message);if(ew&&ew.code!=='PGRST116')console.warn(ew);setRows(r||[]);setSchedule(w||null)}
 const sessions=useMemo(()=>pairEmployeeRecords(rows,nowTick),[rows,nowTick]);const latest=rows[rows.length-1];const working=String(latest?.type||'').toLowerCase()==='entrada';const weekMinutes=sessions.reduce((a,x)=>a+x.minutes,0);const validatedMinutes=sessions.filter(x=>!x.incomplete).reduce((a,x)=>a+x.minutes,0);const scheduleByDay=useMemo(()=>scheduleForEmployee(schedule,user||{}),[schedule,user]);const plannedMinutes=Object.values(scheduleByDay).flat().reduce((a,s)=>a+Math.round(h(s)*60),0);const diff=validatedMinutes-plannedMinutes;
 const byDay=useMemo(()=>{const b={};DAYS.forEach(d=>b[d]={minutes:0,sessions:[]});sessions.forEach(x=>{const d=dayNameFromDate(x.start.created_at);b[d].minutes+=x.minutes;b[d].sessions.push(x)});return b},[sessions]);
 const punctuality=useMemo(()=>{let expected=0,onTime=0;DAYS.forEach(day=>{const slots=scheduleByDay[day]||[];if(!slots.length)return;const first=slots.slice().sort()[0]?.split('-')[0];const entry=byDay[day]?.sessions?.[0]?.start;if(first&&entry){expected++;const d=new Date(entry.created_at);const [hh,mm]=first.split(':').map(Number);const exp=new Date(d);exp.setHours(hh,mm,0,0);if((d-exp)<=10*60000)onTime++}});return expected?Math.round(onTime/expected*100):100},[scheduleByDay,byDay]);
 async function clock(type,method='gps'){if(!user)return;setBusy(true);setMsg('Comprobando ubicación...');let coords=null;if(method==='gps'){try{coords=await new Promise((res,rej)=>navigator.geolocation.getCurrentPosition(res,rej,{enableHighAccuracy:true,timeout:15000,maximumAge:0}))}catch{setBusy(false);setQr(true);return setMsg('GPS no disponible. Usa el QR del bar como respaldo.')}}const payload={p_employee_name:user.name,p_pin:null,p_type:type,p_note:note,p_gps_lat:coords?.coords?.latitude||null,p_gps_lng:coords?.coords?.longitude||null,p_accuracy:coords?.coords?.accuracy||null,p_method:method};let result=await supabase.rpc('registrar_fichaje_portal',{p_employee_id:user.id,p_type:type,p_note:note,p_gps_lat:payload.p_gps_lat,p_gps_lng:payload.p_gps_lng,p_accuracy:payload.p_accuracy,p_method:method});if(result.error&&String(result.error.message).includes('registrar_fichaje_portal'))result=await supabase.rpc('registrar_fichaje_v2',{...payload,p_pin:await currentPin()});setBusy(false);if(result.error)return setMsg('❌ '+result.error.message);const data=result.data;setMsg((data?.ok?'✅ ':'❌ ')+(data?.message||'Fichaje registrado')+(data?.distance_m?` · Distancia ${Math.round(data.distance_m)} m`:''));if(!data?.ok&&data?.outside_radius)setQr(true);if(data?.ok){setNote('');await loadPortal()}}
 async function currentPin(){const{data}=await supabase.from('employees').select('pin').eq('id',user.id).maybeSingle();return data?.pin||''}
 async function requestForgottenExit(){if(!requestTime||!requestReason.trim())return setMsg('Indica la hora y explica el motivo.');setBusy(true);const iso=new Date(`${today()}T${requestTime}:00`).toISOString();let{error}=await supabase.from('clock_adjustment_requests').insert({employee_id:user.id,employee_name:user.name,request_type:'salida_olvidada',requested_at:iso,reason:requestReason.trim(),status:'pendiente'});setBusy(false);if(error)return setMsg('❌ Ejecuta primero el SQL RC 4.0.2 en Supabase: '+error.message);setRequestTime('');setRequestReason('');setMsg('✅ Solicitud enviada. Las horas no contarán hasta que gerencia la apruebe.')}
 async function changePin(){if(!pinForm.current||!pinForm.next||pinForm.next!==pinForm.repeat)return setMsg('Revisa el PIN actual y confirma dos veces el nuevo PIN.');if(pinForm.next.length<4)return setMsg('El nuevo PIN debe tener al menos 4 cifras.');const{data}=await supabase.from('employees').select('pin').eq('id',user.id).maybeSingle();if(String(data?.pin)!==String(pinForm.current))return setMsg('❌ El PIN actual no es correcto.');const{error}=await supabase.from('employees').update({pin:pinForm.next}).eq('id',user.id);if(error)return setMsg('❌ '+error.message);setPinForm({current:'',next:'',repeat:''});setMsg('✅ PIN cambiado correctamente. Gerencia mantiene el control de acceso.')}
 function shareWeek(){let text=`*Mi resumen semanal · ${user.name}*\n`;DAYS.forEach(d=>text+=`${d}: ${hm(byDay[d].minutes)}${byDay[d].sessions.some(x=>x.incomplete)?' (jornada abierta)':''}\n`);text+=`\n*Total fichado: ${hm(weekMinutes)}*\nHoras validadas: ${hm(validatedMinutes)}\nCuadrante: ${hm(plannedMinutes)}\nEstimación: ${(validatedMinutes/60*7).toFixed(2).replace('.',',')} €`;navigator.clipboard?.writeText(text);location.href=`https://wa.me/?text=${encodeURIComponent(text)}`}
 if(!user)return <main className="employeePortal login"><Brand/><section className="employeeLoginCard"><div className="employeeWelcome"><span>👋</span><h2>Tu jornada, tus horas y tu tranquilidad</h2><p>Accede a tu espacio personal para fichar, consultar tus horas y seguir tu semana.</p></div><select value={selected} onChange={e=>setSelected(e.target.value)}><option value="">Selecciona tu nombre</option>{employees.map(e=><option value={e.id} key={e.id}>{e.name}</option>)}</select><input inputMode="numeric" autoComplete="current-password" placeholder="Tu PIN personal" type="password" value={pin} onChange={e=>setPin(e.target.value)} onKeyDown={e=>e.key==='Enter'&&login()}/><button className="employeePrimary" onClick={login} disabled={busy}>{busy?'Entrando...':'Entrar en mi perfil'}</button><p className="msg">{msg}</p><small>Tu PIN es privado. Puedes cambiarlo desde tu perfil.</small></section></main>;
 return <main className="employeeApp"><header className="employeeHeader"><div><small>{getGreeting()}</small><h1>{user.name} 👋</h1></div><button className="employeeLogout" onClick={logout}>Salir</button></header><nav className="employeeTabs">{[['inicio','⌂','Inicio'],['semana','◷','Mi semana'],['evolucion','↗','Evolución'],['perfil','♙','Perfil']].map(([id,ic,l])=><button key={id} className={tab===id?'active':''} onClick={()=>{setTab(id);setMsg('')}}><span>{ic}</span>{l}</button>)}</nav><section className="employeeContent">
 {tab==='inicio'&&<><div className={'employeeStatus '+(working?'working':'off')}><div><span>{working?'🟢 Jornada iniciada':'🌿 Ahora estás fuera'}</span><h2>{working?`Desde ${safeHour(latest.created_at)}`:'Todo preparado para tu próximo turno'}</h2><p>{working?`Llevas ${hm(durationMinutes(nowTick-new Date(latest.created_at).getTime()))} trabajando hoy.`:'Cuando llegues al local, ficha tu entrada por GPS o QR.'}</p></div><div className="employeeClockActions">{!working?<button onClick={()=>clock('entrada')} disabled={busy}>Fichar entrada GPS</button>:<button className="red" onClick={()=>clock('salida')} disabled={busy}>Fichar salida GPS</button>}<button className="soft" onClick={()=>setQr(!qr)}>Usar QR</button></div>{qr&&<div className="employeeQr"><p>Usa el QR físico del bar como método de respaldo.</p>{!working?<button onClick={()=>clock('entrada','qr')}>Entrada QR</button>:<button className="red" onClick={()=>clock('salida','qr')}>Salida QR</button>}</div>}</div><div className="employeeKpis"><article><span>Esta semana</span><b>{hm(weekMinutes)}</b><small>Horas fichadas</small></article><article><span>Cuadrante</span><b>{hm(plannedMinutes)}</b><small>Horas previstas</small></article><article><span>Estimación</span><b>{money(validatedMinutes/60*7)}</b><small>Solo horas cerradas</small></article></div><div className="employeeFriendly card"><h2>{Math.abs(diff)<=15?'👍 Vas perfecto':diff<0?'Tu semana va avanzando':'Horas por revisar'}</h2><p>{plannedMinutes===0?'Aún no hay horas de cuadrante cargadas para esta semana.':Math.abs(diff)<=15?'Tus horas cerradas coinciden prácticamente con el cuadrante.':diff<0?`Llevas ${hm(Math.abs(diff))} menos que lo previsto. Puede ser normal si aún no has terminado la semana.`:`Hay ${hm(diff)} por encima del cuadrante. Gerencia revisará qué tiempo debe validarse.`}</p><div className="employeeProgress"><i style={{width:`${plannedMinutes?Math.min(100,weekMinutes/plannedMinutes*100):0}%`}}></i></div></div><div className="employeeNote card"><h3>Observación del fichaje</h3><textarea placeholder="Opcional: anota algo útil para gerencia" value={note} onChange={e=>setNote(e.target.value)}/></div><div className="employeeForgot card"><h2>¿Olvidaste fichar la salida?</h2><p>No pasa nada. Indica la hora real y el motivo. Gerencia la revisará antes de sumar esas horas.</p><div className="employeeFormRow"><input type="time" value={requestTime} onChange={e=>setRequestTime(e.target.value)}/><input placeholder="Motivo obligatorio" value={requestReason} onChange={e=>setRequestReason(e.target.value)}/></div><button onClick={requestForgottenExit} disabled={busy}>Enviar solicitud de revisión</button></div></>}
 {tab==='semana'&&<div className="employeeWeek"><div className="employeeSectionTitle"><div><small>RESUMEN SEMANAL</small><h2>Así llevas tu semana</h2></div><button onClick={shareWeek}>Compartir por WhatsApp</button></div>{DAYS.map(day=><article className="employeeDay" key={day}><div><b>{day}</b><small>{(scheduleByDay[day]||[]).length?`Previsto: ${(scheduleByDay[day]||[]).join(', ')}`:'Sin turno previsto'}</small></div><div><strong>{hm(byDay[day].minutes)}</strong><span>{byDay[day].sessions.some(x=>x.incomplete)?'🟡 Abierta':byDay[day].minutes?'✅ Cerrada':'—'}</span></div></article>)}<div className="employeeWeekTotal"><span>Total fichado</span><b>{hm(weekMinutes)}</b><small>Validadas/cerradas: {hm(validatedMinutes)}</small></div></div>}
 {tab==='evolucion'&&<div className="employeeEvolution"><div className="employeeSectionTitle"><div><small>MI EVOLUCIÓN</small><h2>Tus estadísticas personales</h2></div></div><div className="employeeStats"><article><span>⏰ Puntualidad</span><b>{punctuality}%</b><small>Según tus entradas y cuadrante</small></article><article><span>✅ Jornadas cerradas</span><b>{sessions.filter(x=>!x.incomplete).length}</b><small>Esta semana</small></article><article><span>🎯 Cumplimiento</span><b>{plannedMinutes?Math.round(validatedMinutes/plannedMinutes*100):0}%</b><small>Horas cerradas frente al cuadrante</small></article><article><span>💶 Estimación semanal</span><b>{money(validatedMinutes/60*7)}</b><small>A razón de 7 €/h validada</small></article></div><div className="employeeFriendly card"><h2>{punctuality>=95?'🌟 Excelente constancia':punctuality>=80?'🙂 Buen progreso':'🌱 Sigue mejorando'}</h2><p>Este espacio es para ayudarte a tener tus horas claras y evitar olvidos. Las horas pendientes o extraordinarias siempre serán revisadas por gerencia.</p></div></div>}
 {tab==='perfil'&&<div className="employeeProfile"><div className="employeeIdentity"><span style={{background:user.color||'#25b99a'}}>{user.name.slice(0,1).toUpperCase()}</span><div><h2>{user.name}</h2><p>Perfil de empleado activo</p></div></div><div className="card"><h2>Cambiar mi PIN</h2><p>Elige un PIN que solo conozcas tú. Gerencia podrá restablecerlo cuando sea necesario.</p><input type="password" inputMode="numeric" placeholder="PIN actual" value={pinForm.current} onChange={e=>setPinForm({...pinForm,current:e.target.value})}/><input type="password" inputMode="numeric" placeholder="Nuevo PIN" value={pinForm.next} onChange={e=>setPinForm({...pinForm,next:e.target.value})}/><input type="password" inputMode="numeric" placeholder="Repetir nuevo PIN" value={pinForm.repeat} onChange={e=>setPinForm({...pinForm,repeat:e.target.value})}/><button onClick={changePin}>Guardar nuevo PIN</button></div><div className="card"><h2>Privacidad y control</h2><p>Tu perfil solo se abre con tu PIN. Gerencia conserva el control administrativo para darte acceso, bloquearlo o restablecerlo.</p></div></div>}
 {msg&&<div className="employeeToast">{msg}</div>}</section></main>
}
function ClockPage(){return <EmployeePortal/>}
