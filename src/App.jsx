...
function UnifiedProfitability(){
 const[mode,setMode]=useState('hoy'),[date,setDate]=useState(today()),[state,setState]=useState({tickets:[],lines:[],costMap:new Map(),real:null}),[loading,setLoading]=useState(false),[fixed,setFixed]=useState([]),[variable,setVariable]=useState([]),[fixedForm,setFixedForm]=useState({name:'',category:'Alquiler',monthly_amount:'',start_date:today(),end_date:'',notes:'',active:true}),[variableForm,setVariableForm]=useState({name:'',category:'Otros',expense_date:today(),amount:'',payment_method:'Efectivo',notes:''});
 ...
 async function load(){if(!supabase)return;setLoading(true);try{const [{tickets,lines},costMap,real,{data:f},{data:v}]=await Promise.all([loadSalesRange(r.from,r.to),loadProfitabilityCostMap(),loadRealProfitability(supabase,r.from,r.to,[]),supabase.from('business_fixed_expenses').select('*').order('active',{ascending:false}).order('start_date',{ascending:false}).limit(1000),supabase.from('business_variable_expenses').select('*').order('expense_date',{ascending:false}).limit(2000)]);setState({tickets,lines,costMap,real});setFixed(f||[]);setVariable(v||[])}finally{setLoading(false)}}
 ...
 return <div className="unifiedProfit"><div className="card hero"><div><span className="sectionEyebrow">CORE DE RENTABILIDAD · FUENTE ÚNICA</span><h2>Resultado real del negocio</h2><p>{r.label}. El mismo cálculo alimenta Inteligencia, Dashboard y contabilidad.</p></div><div className="row controls">{buttons.map(([id,l])=><button key={id} className={mode===id?'active':''} onClick={()=>setMode(id)}>{l}</button>)}<input type="date" value={date} onChange={e=>{setDate(e.target.value);setMode('fecha')}}/><button onClick={load}>{loading?'Calculando…':'Actualizar'}</button></div></div>
 <div className="grid unifiedProfitKpis"><div className="card kpi"><span>Ventas</span><b>{money(daily.total)}</b><small>{daily.tickets} tickets · {money(daily.ticket_medio)} medio</small></div><div className="card kpi"><span>Coste productos</span><b>{money(products.cost)}</b><small>{money(products.estimatedRevenue)} con coste 1/3 estimado</small></div><div className="card kpi"><span>Personal devengado</span><b>{money(real.laborAccrued)}</b><small>{Number(real.hours||0).toFixed(1)} h según cuadrante y coste individual</small></div><div className="card kpi"><span>Gastos fijos</span><b>{money(real.fixed)}</b><small>Prorrateados por días naturales</small></div><div className="card kpi"><span>Gastos variables</span><b>{money(real.variable)}</b><small>Movimientos del periodo</small></div><div className="card kpi"><span>Margen bruto</span><b>{money(gross)}</b><small>Ventas menos producto</small></div><div className="card kpi"><span>Margen operativo</span><b>{money(operating)}</b><small>Después de personal</small></div><div className="card kpi realProfitKpi"><span>Beneficio real</span><b className={profit>=0?'ok':'bad'}>{money(profit)}</b><small>{margin.toFixed(1)}% sobre ventas</small></div></div>
 ...
 <div className="grid"><section className="card"><h2>Personal imputado</h2>{(real.details?.labor||[]).length?(real.details.labor||[]).map(x=><p className="profitDetailRow" key={x.employee_id||x.employee_name}><span>{x.employee_name}</span><b>{Number(x.hours).toFixed(1)} h × {money(x.hourly_cost)} = {money(x.cost)}</b></p>):<p>Sin horas previstas en el cuadrante para este periodo.</p>}</section><section className="card"><h2>Fijos imputados al periodo</h2>{(real.details?.fixed||[]).length?(real.details.fixed||[]).map(x=><p className="profitDetailRow" key={x.id}><span>{x.name} · {x.days} días</span><b>{money(x.imputed)}</b></p>):<p>Sin gastos fijos activos.</p>}</section></div></div>
}
...
function buildEmployeeSessions(employee,rows){
 const mine=(rows||[]).filter(r=>employeeMatches(employee,r.employee_id)||slugName(r.employee_name)===slugName(employee.name)).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
 const sessions=[];let open=null;
 mine.forEach(r=>{const type=String(r.type||'').toLowerCase();if(type==='entrada'){if(open)sessions.push({start:open,end:r,minutes:minutesBetween(open.created_at,r.created_at),open:false,orphan:true,inferred:true});open=r}else if(type==='salida'){if(open){sessions.push({start:open,end:r,minutes:minutesBetween(open.created_at,r.created_at),open:false});open=null}else sessions.push({start:null,end:r,minutes:0,open:false,orphan:true})}});
 if(open)sessions.push({start:open,end:null,minutes:minutesBetween(open.created_at,new Date()),open:true});
 return sessions;
}
...
function StaffControl({initialTab='plantilla'}){
 ...
 const totals=visible.reduce((a,s)=>({working:a.working+(s.open?1:0),week:a.week+s.weekMinutes,month:a.month+s.monthMinutes,planned:a.planned+s.planned.minutes,cost:a.cost+s.planned.minutes/60*Number(s.employee.hourly_rate||7),late:a.late+s.late5+s.late10,without:a.without+s.without}),{working:0,week:0,month:0,planned:0,cost:0,late:0,without:0});
 async function createEmployee(){const n=newEmployee.name.trim(),p=String(newEmployee.pin||'').trim();if(!n||!p)return alert('Indica nombre y PIN');const payload={...newEmployee,name:n,pin:p,hourly_rate:Number(newEmployee.hourly_rate||7)};const {data,error}=await supabase.from('employees').insert(payload).select('*').single();if(error)return alert(error.message);setCreateOpen(false);setNewEmployee({name:'',pin:'',role:'Sala',hourly_rate:7,color:EMP_COLORS[(employees.length+1)%EMP_COLORS.length],can_clock:true,active:true});await load();alert(`${data.name} ya está disponible en Cuadrantes y en el portal de fichaje.`)}
 ...
 async function saveEmployee(){if(!draft?.name?.trim())return alert('El nombre no puede quedar vacío');const patch={name:draft.name.trim(),pin:String(draft.pin||''),role:draft.role||'Sala',color:draft.color||'#31b9d4',can_clock:draft.can_clock!==false,active:draft.active!==false,hourly_rate:Number(draft.hourly_rate||7)};const {data,error}=await supabase.from('employees').update(patch).eq('id',draft.id).select('*').single();if(error)return alert(error.message);await migrateEmployeeScheduleReferences(data,draft.oldName);setSelected(null);setDraft(null);await load();alert('Ficha guardada y sincronizada con el cuadrante.')}
 async function closeManual(stat){const open=stat.open?.start;if(!open)return;const suggested=new Date().toISOString().slice(0,16);const value=prompt(`Hora de salida real para ${stat.employee.name}`,suggested);if(!value)return;const reason=prompt('Motivo del cierre manual','Olvido de fichaje')||'Cierre manual por gerencia';const {error}=await supabase.from('clock_records').insert({employee_id:stat.employee.id,employee_name:stat.employee.name,type:'salida',method:'manual',inside_radius:true,note:`SALIDA MANUAL POR GERENCIA · ${reason}`,created_at:new Date(value).toISOString()});if(error)return alert(error.message);await load()}
 async function addManualClock(employee,type){const value=prompt(`${type==='entrada'?'Entrada':'Salida'} manual para ${employee.name}`,new Date().toISOString().slice(0,16));if(!value)return;const reason=prompt('Motivo','Corrección manual por gerencia')||'Corrección manual';const {error}=await supabase.from('clock_records').insert({employee_id:employee.id,employee_name:employee.name,type,method:'manual',inside_radius:true,note:`${type.toUpperCase()} MANUAL · ${reason}`,created_at:new Date(value).toISOString()});if(error)return alert(error.message);await load()}
