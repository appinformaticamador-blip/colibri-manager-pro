...
function UnifiedProfitability(){
 ...
 const productRows=useMemo(()=>{const m=new Map();(state.lines||[]).forEach(l=>{const f=lineFinancials(l,state.costMap);const k=f.code||l.descripcion||'Sin código';const x=m.get(k)||{name:f.info?.name||l.descripcion||k,qty:0,revenue:0,cost:0,estimated:0};x.qty+=f.qty;x.revenue+=f.revenue;x.cost+=f.cost;if(!f.known)x.estimated+=f.revenue;m.set(k,x)});return[...m.values()].sort((a,b)=>b.revenue-a.revenue)},[state.lines,state.costMap]);
 const details={ventas:{title:'Detalle de ventas',rows:[['Periodo',`${r.from} → ${addDays(r.to,-1)}`],['Tickets',daily.tickets],['Ticket medio',money(daily.ticket_medio)],['Efectivo',money(daily.efectivo)],['Tarjeta',money(daily.tarjeta)],['Total',money(daily.total)]]},productos:{title:'Coste de productos vendidos',rows:[['Coste total',money(products.cost)],['Venta con coste estimado 1/3',money(products.estimatedRevenue)],...productRows.slice(0,80).map(x=>[`${x.name} · ${x.qty.toFixed(2)} uds`,`${money(x.cost)} coste · ${money(x.revenue)} venta${x.estimated?' · estimado':''}`])]},personal:{title:'Personal devengado',rows:[['Horas imputadas',`${Number(real.hours||0).toFixed(2)} h`],['Coste total',money(real.laborAccrued)],...(real.details?.labor||[]).map(x=>[x.employee_name,`${Number(x.hours||0).toFixed(2)} h × ${money(x.hourly_cost)} = ${money(x.cost)}`])]},fijos:{title:'Gastos fijos imputados',rows:[['Total',money(real.fixed)],...(real.details?.fixed||[]).map(x=>[`${x.name} · ${x.days} días`,money(x.imputed)])]},variables:{title:'Gastos variables del periodo',rows:[['Total',money(real.variable)],...(real.details?.variable||[]).map(x=>[`${x.expense_date||x.date||''} · ${x.name||x.description||'Gasto'}`,money(x.amount||x.total)])]},bruto:{title:'Margen bruto',rows:[['Ventas',money(daily.total)],['Coste productos',`- ${money(products.cost)}`],['Margen bruto',money(gross)],['Porcentaje',daily.total?`${(gross/daily.total*100).toFixed(1)}%`:'0%']]},operativo:{title:'Margen operativo',rows:[['Margen bruto',money(gross)],['Personal',`- ${money(real.laborAccrued)}`],['Margen operativo',money(operating)]]},beneficio:{title:'Beneficio real',rows:[['Margen operativo',money(operating)],['Gastos fijos',`- ${money(real.fixed)}`],['Gastos variables',`- ${money(real.variable)}`],['Beneficio real',money(profit)],['Margen real',`${margin.toFixed(1)}%`]]}};
 ...
 {detail&&<div className="modal unifiedProfitModalOverlay" onClick={()=>setDetail(null)}><div className="modalbox unifiedProfitDetail" onClick={e=>e.stopPropagation()}><div className="row between unifiedProfitDetailHeader"><div><span className="sectionEyebrow">DETALLE DEL RESULTADO REAL</span><h2>{detail.title}</h2></div><button className="ghost" onClick={()=>setDetail(null)}>Cerrar</button></div><div className="profitDetailList">{detail.rows.map((x,i)=><p className="profitDetailRow" key={i}><span>{x[0]}</span><b>{x[1]}</b></p>)}</div></div></div>}</div>
}
