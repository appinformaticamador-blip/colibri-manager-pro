...
import type { CostControlEvent, MasterItem, NumierArticle, NumierLink, PurchaseItem, Recipe, RecipeIngredient } from './types'
...
export default function EditableMargins({ supabase, links, masterItems, recipes, recipeIngredients, purchaseItems, numierCatalog, onRefresh, currentUser = 'Alfonso' }: Props) {
  ...
  const rows = useMemo(() => buildMarginProducts({ links, masters: masterItems, recipes, ingredients: recipeIngredients, purchaseItems, numierCatalog }), [links, masterItems, numierCatalog, purchaseItems, recipeIngredients, recipes])
  ...
  async function control(action: 'set_manual' | 'restore_auto' | 'exclude' | 'include') {
    ...
    setBusy(true)
    const { error } = await supabase.rpc('set_profitability_cost_control', {
    ...
    if (error) { setBusy(false); return notify('error', `${error.message}. Ejecuta la migración RC 3.9.2.`) }
  ...
  return <section className="editableMargins">
    ...
    <div className="marginCards editableMarginCards">{filtered.map(row => <button className={`editableMarginCard ${row.excluded ? 'excluded' : ''} ${!row.complete ? 'incomplete' : ''}`} key={row.id} onClick={() => open(row)}>
      <div><b>{row.productName}</b><small>{row.articleName} · {row.articleCode} · {row.entityType === 'recipe' ? 'Escandallo' : 'Venta directa'}</small>{row.linkedArticles.length > 1 && <em>{row.linkedArticles.length} vínculos NUMIER</em>}</div>
    ...
    {selected && <div className="modal" onClick={() => !busy && setSelected(null)}><div className="card marginEditorModal" onClick={event => event.stopPropagation()}>
      <div className="row between marginEditorTitle"><div><span className="sectionEyebrow">{selected.entityType === 'recipe' ? 'ESCANDALLO' : 'PRODUCTO DIRECTO'}</span><h2>{selected.productName}</h2><p>{selected.articleName} · Código {selected.articleCode}</p></div><button className="red" onClick={() => setSelected(null)}>Cerrar</button></div>
