...
function Manager(){
 ...
 const tabs=[['dashboard','⌂','Dashboard'],['servicio','◉','Centro mando'],['inteligencia','✦','Inteligencia'],['tpv','▣','TPV'],['gestoria','▤','Gestoría'],['rentabilidad','€','Rentabilidad'],['almacen','▥','Almacén'],['asistente','✧','IA del negocio'],['empleados','♟','Personal'],['cuadrantes','▦','Cuadrantes'],['comparador','⇄','Comparador'],['config','⚙','Configuración']];
...
function buildEmployeeSessions(employee,rows){
 ...
 mine.forEach(r=>{const type=String(r.type||'').toLowerCase();if(type==='entrada'){if(open)sessions.push({start:open,end:null,minutes:minutesBetween(open.created_at,new Date()),open:true,orphan:true});open=r}else if(type==='salida'){if(open){sessions.push({start:open,end:r,minutes:minutesBetween(open.created_at,r.created_at),open:false});open=null}else sessions.push({start:null,end:r,minutes:0,open:false,orphan:true})}});
