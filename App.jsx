...
function employeeClockStats(employee,rows){
 const keys=new Set([String(employee.id),slugName(employee.name)]);const mine=(rows||[]).filter(r=>keys.has(String(r.employee_id))||slugName(r.employee_name)===slugName(employee.name)).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
 const now=new Date(),weekAgo=new Date(now);weekAgo.setDate(now.getDate()-7);const monthStart=new Date(now.getFullYear(),now.getMonth(),1);let total=0,week=0,month=0,days=new Set(),open=null;
 for(let i=0;i<mine.length;i++){const r=mine[i];if(String(r.type).toLowerCase()!=='entrada')continue;const exit=mine.slice(i+1).find(x=>String(x.type).toLowerCase()==='salida');const end=exit?.created_at||new Date().toISOString();const mins=minutesBetween(r.created_at,end);total+=mins;if(new Date(r.created_at)>=weekAgo)week+=mins;if(new Date(r.created_at)>=monthStart)month+=mins;days.add(String(r.created_at).slice(0,10));if(!exit)open=r;}
 return {total,week,month,days:days.size,open};
}
function formatHours(mins){return `${Math.floor(mins/60)} h ${String(mins%60).padStart(2,'0')} min`}
function Employees(){
 const[employees,setEmployees]=useState([]),[rows,setRows]=useState([]),[selected,setSelected]=useState(null),[draft,setDraft]=useState(null),[name,setName]=useState(''),[pin,setPin]=useState(''),[loading,setLoading]=useState(false);
 useEffect(()=>{load()},[]);
 async function load(){if(!supabase)return;setLoading(true);const [{data:emps,error:e1},{data:clocks,error:e2}]=await Promise.all([supabase.from('employees').select('*').order('name'),supabase.from('clock_records').select('*').order('created_at',{ascending:false}).limit(2500)]);setLoading(false);if(e1)return alert(e1.message);if(e2)console.warn(e2);setEmployees(emps||[]);setRows(clocks||[])}
 async function add(){if(!name.trim()||!pin.trim())return alert('Indica nombre y PIN');const color=EMP_COLORS[employees.length%EMP_COLORS.length];const{error}=await supabase.from('employees').insert({name:name.trim(),pin:pin.trim(),role:'Sala',color,can_clock:true,active:true,hourly_rate:7});if(error)return alert(error.message);setName('');setPin('');await load()}
 function openEdit(e){setSelected(e.id);setDraft({...e,role:e.role||'Sala',hourly_rate:Number(e.hourly_rate||7)})}
 async function save(){if(!draft?.name?.trim())return alert('El nombre no puede quedar vacío');const patch={name:draft.name.trim(),pin:String(draft.pin||''),role:draft.role||'Sala',color:draft.color||'#31b9d4',can_clock:draft.can_clock!==false,active:draft.active!==false,hourly_rate:Number(draft.hourly_rate||0)};const{error}=await supabase.from('employees').update(patch).eq('id',draft.id);if(error)return alert(error.message);setSelected(null);setDraft(null);await load();alert('Ficha guardada. El cambio se reflejará en Cuadrantes y Fichajes.')}
 async function toggle(e){const{error}=await supabase.from('employees').update({active:!e.active}).eq('id',e.id);if(error)return alert(error.message);await load()}
 return <div className="employeesControl"><div className="card employeeHero"><div><h2>Centro de control de empleados</h2><p>Una única ficha alimenta cuadrantes, fichajes, puntualidad y costes de personal.</p></div><button onClick={load}>{loading?'Actualizando…':'Actualizar'}</button></div><div className="card employeeCreate"><h3>Nuevo empleado</h3><div className="row"><input placeholder="Nombre" value={name} onChange={e=>setName(e.target.value)}/><input placeholder="PIN" inputMode="numeric" value={pin} onChange={e=>setPin(e.target.value)}/><button onClick={add}>Añadir empleado</button></div></div><div className="employeeCards">{employees.map(e=>{const st=employeeClockStats(e,rows);return <article className={'employeeProfile '+(e.active?'':'inactive')} key={e.id}><header><span className="employeeAvatar" style={{background:e.color}}>{String(e.name||'?').slice(0,1).toUpperCase()}</span><div><h3>{e.name}</h3><p>{e.role||'Sala'} · {e.active?'Activo':'Inactivo'}</p></div><span className={'statusDot '+(st.open?'working':'')}>{st.open?'Trabajando':'Sin turno abierto'}</span></header><div className="employeeStats"><div><span>Semana</span><b>{formatHours(st.week)}</b></div><div><span>Mes</span><b>{formatHours(st.month)}</b></div><div><span>Jornadas</span><b>{st.days}</b></div><div><span>Coste/hora</span><b>{Number(e.hourly_rate||7).toLocaleString('es-ES',{style:'currency',currency:'EUR'})}</b></div></div>{st.open&&<p className="openShift">Entrada {new Date(st.open.created_at).toLocaleString('es-ES')} · {formatHours(minutesBetween(st.open.created_at,new Date()))}</p>}<footer><button onClick={()=>openEdit(e)}>Editar ficha</button><button className={e.active?'red':''} onClick={()=>toggle(e)}>{e.active?'Desactivar':'Activar'}</button></footer>{selected===e.id&&draft&&<div className="employeeEditor"><h4>Ficha de {e.name}</h4><label>Nombre<input value={draft.name||''} onChange={x=>setDraft({...draft,name:x.target.value})}/></label><label>PIN<input inputMode="numeric" value={draft.pin||''} onChange={x=>setDraft({...draft,pin:x.target.value})}/></label><label>Puesto<select value={draft.role||'Sala'} onChange={x=>setDraft({...draft,role:x.target.value})}><option>Sala</option><option>Barra</option><option>Cocina</option><option>Gerencia</option></select></label><label>Coste por hora<input type="number" min="0" step="0.01" value={draft.hourly_rate||0} onChange={x=>setDraft({...draft,hourly_rate:x.target.value})}/></label><label>Color<input type="color" value={draft.color||'#31b9d4'} onChange={x=>setDraft({...draft,color:x.target.value})}/></label><label className="checkLine"><input type="checkbox" checked={draft.can_clock!==false} onChange={x=>setDraft({...draft,can_clock:x.target.checked})}/> Puede fichar</label><div className="row"><button onClick={save}>Guardar cambios</button><button className="ghost" onClick={()=>{setSelected(null);setDraft(null)}}>Cancelar</button></div></div>}</article>})}</div></div>
}
...
function ClockPanel(){
 const[rows,setRows]=useState([]),[employees,setEmployees]=useState([]),[open,setOpen]=useState([]),[filter,setFilter]=useState('todos'),[range,setRange]=useState('30');
 useEffect(()=>{load()},[]);
 async function load(){if(!supabase)return;const[{data,error},{data:emps}]=await Promise.all([supabase.from('clock_records').select('*').order('created_at',{ascending:false}).limit(2500),supabase.from('employees').select('*').eq('active',true).order('name')]);if(error)return alert(error.message);const list=data||[];setRows(list);setEmployees(emps||[]);const latest=new Map();list.forEach(r=>{const key=String(r.employee_id||slugName(r.employee_name));if(!latest.has(key))latest.set(key,r)});setOpen([...latest.values()].filter(r=>String(r.type).toLowerCase()==='entrada'))}
 async function closeManual(r){const suggested=new Date().toISOString().slice(0,16);const value=prompt(`Hora de salida real para ${r.employee_name}`,suggested);if(!value)return;const reason=prompt('Motivo del cierre manual','Olvido de fichaje')||'Cierre manual por gerencia';const{error}=await supabase.from('clock_records').insert({employee_id:r.employee_id,employee_name:r.employee_name,type:'salida',method:'manual',inside_radius:true,note:`SALIDA MANUAL POR GERENCIA · ${reason}`,created_at:new Date(value).toISOString()});if(error)return alert(error.message);await load()}
 const days=Number(range||30),cut=new Date(Date.now()-days*86400000);const expected=expectedStartMapToday();const filtered=rows.filter(r=>new Date(r.created_at)>=cut&&(filter==='todos'||String(r.employee_id)===filter||slugName(r.employee_name)===slugName(employees.find(e=>String(e.id)===filter)?.name)));
 const entries=filtered.filter(r=>String(r.type).toLowerCase()==='entrada');const punctual=entries.filter(r=>punctualityFor(r,expected).cls==='ontime').length,late5=entries.filter(r=>punctualityFor(r,expected).cls==='late5').length,late10=entries.filter(r=>punctualityFor(r,expected).cls==='late10').length,without=entries.filter(r=>punctualityFor(r,expected).cls==='none').length;
 return <div className="clockControl"><div className="card clockToolbar"><div><h2>Control horario y puntualidad</h2><p>Resumen intuitivo de presencia, retrasos e incidencias.</p></div><div className="row"><select value={filter} onChange={e=>setFilter(e.target.value)}><option value="todos">Todos los empleados</option>{employees.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select><select value={range} onChange={e=>setRange(e.target.value)}><option value="7">7 días</option><option value="30">30 días</option><option value="90">90 días</option></select><button onClick={load}>Actualizar</button></div></div><div className="clockKpis"><article><span>Trabajando ahora</span><b>{open.length}</b></article><article><span>Puntuales</span><b>{punctual}</b></article><article><span>Retraso 5–10 min</span><b>{late5}</b></article><article><span>Retraso +10 min</span><b>{late10}</b></article><article><span>Sin turno previsto</span><b>{without}</b></article></div><div className="card"><h2>Fichajes abiertos</h2>{open.length===0?<p>✅ No hay turnos abiertos.</p>:<div className="openClockGrid">{open.map(r=><article key={r.id}><div><b>{r.employee_name}</b><span>Entrada: {new Date(r.created_at).toLocaleString('es-ES')}</span><strong>{formatHours(minutesBetween(r.created_at,new Date()))}</strong></div><button className="red" onClick={()=>closeManual(r)}>Cerrar turno</button></article>)}</div>}</div><div className="card wide"><h2>Historial de fichajes</h2><div className="clockHistory">{filtered.slice(0,300).map(r=>{const p=punctualityFor(r,expected);return <article key={r.id} className={'clockHistoryRow '+p.cls}><div><b>{r.employee_name}</b><span>{new Date(r.created_at).toLocaleString('es-ES')}</span></div><strong>{String(r.type).toLowerCase()==='entrada'?`${p.icon} ${p.label}`:'🔴 Salida'}</strong><span>{r.method||'—'}</span><small>{r.note||''}</small></article>})}</div></div></div>
}
